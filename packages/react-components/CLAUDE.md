...
│   ├── button.tsx
│   ├── page-section.tsx                # ページのコンテナ（main/section 切替）
│   ├── page-heading.tsx                # ページの見出し（h1〜h6 切替）
│   └── ...
...
ビュー（V1〜V15）は **ページクロム（見出し・コンテナ要素）を持たない**。ビューが返すのは「そのビュー固有のコンテンツ」（リスト、フォーム、ステート表示等）のみ。ページのアウトライン（`<main>` / `<section>` / `<h1>`〜`<h6>`）は consumer（apps/web 等）が組み立てる責務。
...
理由:
- ビューが見出しレベル（h1 / h2 等）を固定できない（同じビューが別ページの一部として埋め込まれる可能性）
- ビューがコンテナ要素（main / section）を固定できない（呼び出し元のドキュメントアウトラインに依存）
- タイトル文言（「プログラム」等）はアプリ層で再利用や調整が起こりうる

`PageSection` / `PageHeading` プリミティブは `primitive/` に配置し、すべてのビューが共通で使える。

### 配置基準（上から順に判定）

1. V1〜V15 のどれかに対応する → `src/view/{ビュー名}/{ビュー名}.tsx`
2. ビューの状態バリアント（loading / error 等、normal とは異なる構造を持つ） → `src/view/{ビュー名}/{ビュー名}-{状態}.tsx`（`index.ts` から再エクスポート）
3. 特定ビュー専用のサブコンポーネント → `src/view/{ビュー名}/` 配下にコロケーション（`index.ts` から再エクスポートしない）
4. 複数ビューで使われるドメイン部品（昇格後） → `src/view/` 直下
5. ドメイン用語（Program / Day / ExercisePlan / SetPlan / Workout / Exercise / 1RM など）を含まない汎用部品 → `src/primitive/`
...
- `src/view/index.ts`: 各ビューフォルダを `export *` で公開（フォルダ解決で `view/{ビュー名}/index.ts` バレルを拾う）
- `src/view/{ビュー名}/index.ts`: ビュー本体（`{ビュー名}.tsx`）と状態バリアント（`{ビュー名}-loading.tsx` 等）を再エクスポート。**サブコンポーネント（`{ビュー名}-item.tsx` 等）は再エクスポートしない**（internal）
