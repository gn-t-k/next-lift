...
└── views/                              # ビュー層（V1〜V15の単位ビュー）
    ├── program-list/
    │   ├── index.ts                    # バレル: ビュー本体と状態バリアントを再エクスポート
    │   ├── program-list.tsx            # V1 プログラム一覧（通常状態）
    │   ├── program-list-loading.tsx    # V1 ローディング状態（独立コンポーネント）
    │   ├── program-list-error.tsx      # V1 エラー状態（独立コンポーネント）
...
ファイル命名は `coding-style.md` の「ファイル名と export する関数/コンポーネント名を揃える」原則に従う（`program-list.tsx` → `ProgramList`）。`index.ts` はバレル専用（コンポーネント定義を含まない）。

プリミティブも `primitives/{name}/{name}.tsx` のフォルダコロケーション形式で配置する。これによりストーリー（`{name}.stories.tsx`）や、特定プリミティブ専用のサブコンポーネント（`modal/dialog.tsx` のような internal 部品）を同じフォルダにまとめられる。
...
5. 上記の判別基準を満たす汎用部品 → `src/primitives/{名前}/{名前}.tsx`

ドメイン用語とは、Next Lift のビジネスドメイン（トレーニング計画・実施・振り返り・種目マスタ・1RM）に固有の概念名を指す。具体例: Program / Day / ExercisePlan / SetPlan / Workout / Exercise / 1RM。

CRUD 操作（Create / Edit / Delete）やフォーム要素（Button / Field / Selector）のような、フィットネス領域に依存しない UI / 操作概念はドメイン用語に含まない。

判定の目安: 別ジャンルのアプリ（ToDo / 家計簿 / レシピ等）に名前ごと持っていって意味が通るか。通る → `primitives/`、通らない → `views/`。例: `CreateAffordance` は「項目を新規追加する破線枠のアフォーダンス」として ToDo でもレシピでも意味が通る → primitive。`SetPlanRow` は「セット計画の行」で他ドメインで意味が通らない → `views/program-detail/`。

### 公開エクスポートの方針

利用側に提示するのは **ビュー（状態バリアント含む）と、ビュー外でも単独利用が想定されるプリミティブ** だけ。具体的には:

- `src/primitives/index.ts`: 各プリミティブフォルダを `export *` で公開（フォルダ解決で `primitives/{名前}/index.ts` バレルを拾う）
- `src/primitives/{名前}/index.ts`: ビュー本体（`{名前}.tsx`）を再エクスポート。**internal なサブコンポーネント（例: `modal/dialog.tsx`）は再エクスポートしない**
- `src/views/index.ts`: 各ビューフォルダを `export *` で公開（フォルダ解決で `views/{ビュー名}/index.ts` バレルを拾う）
- `src/views/{ビュー名}/index.ts`: ビュー本体（`{ビュー名}.tsx`）と状態バリアント（`{ビュー名}-loading.tsx` 等）を再エクスポート。**サブコンポーネント（`{ビュー名}-item.tsx` 等）は再エクスポートしない**（internal）
