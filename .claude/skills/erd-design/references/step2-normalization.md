...
> **導出項目（他の属性から計算で求められる項目）の判断は Step 4 で扱う。** このステップでは繰り返しと値の重複の排除、およびNULLableカラムの解消に集中する。

**問い3: NULLableなカラムはないか？**

NULLableカラムは「値がない」「未知」「該当しない」の区別がつかず、クエリで予期しない結果を招く。NULLableカラムが残っていたら、ステップ1で検討したテーブル分離が漏れていないか再確認する。

| before | after | 理由 |
|---|---|---|
| notifications に `read_at` (NULL許容) | `notification_reads` テーブルに分離 | 「未読」をNULLで表現せず、「既読イベントの有無」で表現 |
| orders に `cancelled_at` (NULL許容) | `order_cancellations` テーブルに分離 | キャンセル理由など固有属性も持てる |
| users に `deleted_at` (NULL許容) | `user_deletions` テーブルに分離 | 論理削除のNULLフラグを排除 |
...
- [ ] NULLableカラムが残っていないか（残っている場合、別テーブルへの分離を検討したか）
