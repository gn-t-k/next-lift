...
各属性について「この値は変更・削除されることがあるか？」を問い、変更・削除される場合はイベントテーブル分離とUPDATE/DELETEの選択肢を比較する。

**設計判断の記述ルール:**

- 「原則に従う」「原則に反する」は理由にならない。各選択肢のメリット・デメリットを具体的に列挙し、どちらを選ぶか判断する
- RDBのenum型やcheck制約は使わない。値の追加にマイグレーションが必要で保守コストが高い。アプリ側のバリデーションで管理する

**UPDATE/DELETE管理を選択する前の確認チェック:**
...
すべてクリアした場合でも、以下をユーザーに提示して判断を委ねる:
1. 将来イベントテーブルが必要になる**具体的なシナリオ**を挙げる
2. **後方互換性**: 後からイベントテーブルに移行可能か評価する

**ステータスカラムやNULLableカラムが出現したら、イベントテーブル分離と現状維持のpros/consを比較して判断する。**

### 「現要件では不要」と判断する前に

エンティティや属性を「現要件では不要」として排除する場合、以下を検討してからユーザーに提示する:
...
- [ ] 同じ概念を別名で重複抽出していないか
- [ ] UPDATE/DELETEで管理するカラムに正当化理由があるか
- [ ] NULLableカラムを別テーブルに分離する検討をしたか
