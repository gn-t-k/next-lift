...
- ステータス: **確定**（2026-04-30、設計判断 #53 で V14 撤廃に伴い 2-2-2 / 2-2-3 を削除）
- 現在のフェーズ: 5/5 完了
- 葉タスク: 85件（うち 1-1, 1-2, 1-3, 1-4, 2-1-1, 2-1-2, 2-1-3, 2-1-4, 2-1-5, 2-1-6, 2-1-7, 2-2-1, 2-3-1, 2-3-2 完了）
- 未展開ノード: 0件
- 次のアクション: 2-3-3 Day未作成の空状態ストーリー
...
| 2 | プログラム計画フロー | 1 | 中核ドメインであり、プリミティブ層の検証にもなる。V2→V3、V3→V4の順で積み上げ (V14 撤廃: 設計判断 #53) |
...
##### 2-2. ~~V14 プログラム作成~~ (撤廃: 設計判断 #53)

V14 (プログラム作成) は独立ビューとして作らず、プログラム新規作成 (UC_A_1) は V2 のインライン編集に統合する。新規作成 (V14 のフォーム送信) と既存編集 (V2 のインライン編集) で同じドメイン操作 (プログラム名入力) に異なる UI が当てられていた non-uniformity を解消するため (`design-principles.md` 「発見の設計: 一貫性」と整合)。本セクションのうち 2-2-1 (TextField プリミティブ) は完了済で、V2 のインライン編集等で引き続き利用する。
...
- [x] 2-3-1. Tabs プリミティブを追加
  ...
  - **意見の剥がし**: Intent UI テンプレが提供する `SelectionIndicator` (アニメーション付き選択インジケーター) と `TabListContext` を撤去し、`selected:` 状態で `::after` 疑似要素のシンプルな下線/側線に置き換え。`design-principles.md` の「アニメーション控えめ」方針と整合。`TabScrollArea` (横スクロールラッパー) も YAGNI で除外
...
- [x] 2-3-2. V2 骨格 + Day一覧 + 通常状態ストーリー
  ...
  - **新規作成直後の状態**: V1 → V2 直接遷移直後は name が空のことがある (設計判断 #53)。骨格段階では空 name でも崩れないレイアウトを確認。プレースホルダー表示の具体仕様は 2-3-4 で確定
  - **実装方針**: `src/views/program-detail/` に `program-detail.tsx` (`ProgramDetail`) と `day-tab-list.tsx` (`DayTabList`) をコロケーション配置。views-and-navigation.md L171「プログラム → Day = 概要一覧（タブ/横スクロール）」に従い、Day 一覧は 2-3-1 で導入した Tabs プリミティブで表現
  ...
  - **見出しレベル**: ProgramDetail はプログラム名を `<h2>` で表示（PageHeading は consumer 側で `<PageHeading as="h1">プログラム</PageHeading>` を組む前提）。ビューが見出しレベルを固定する点は CLAUDE.md の「ビューはページクロムを持たない」原則と若干緊張するが、V2 はオブジェクト名表示が主で、別ページに埋め込むユースケースが views-and-navigation.md 上に存在しないため許容。インライン編集 (2-3-4) で要素タイプは `<input>` 等に置き換わる
  - **空 name 時のレイアウト**: `min-h-[1lh]` を `<h2>` に付与し、name が空文字でも見出し行の高さが保たれることを確認。プレースホルダー文字列・視覚ヒント (under-line / dashed border 等) は 2-3-4 で確定
  - **Day タブのパネル内容**: 骨格段階では各 TabPanel に「Day の詳細を編集 →」リンク (`detailHref`) のみ配置。Day 概要 (種目数等) や種目計画リストは後続タスク (2-3-5, V3 統合) で詰める
  - **ストーリー**: `MultipleDays` (Day 4 件 + メタあり) / `NoMeta` (メタ null) / `NewlyCreatedEmptyName` (name 空 + meta null) / `LongProgramName` (name の長文ラップ確認) の 4 本。`PageSection width="wide"` + `PageHeading as="h1">プログラム` の decorator は V1 と同形に統一
- [ ] 2-3-3. Day未作成の空状態ストーリー
  - 説明: Day 0件時の表示とDay追加への誘導
  - 完了条件: 空状態ストーリーが表示
- [ ] 2-3-4. プログラム名/メタ情報のインライン編集 + ストーリー
  - 説明: 各フィールドのインライン編集。編集中/非編集の状態遷移。設計判断 #53 により**新規作成 (UC_A_1) と既存編集 (UC_A_6) の両方で同じインライン編集 UI を使う**ため、name 空状態 (新規作成直後) のプレースホルダーと初回入力フローもここで確定する
  - 完了条件: 編集中と確定後のストーリーが両方表示。空状態 (プレースホルダー表示) からの初回入力フローもストーリー化
  - **発見性の仕様**: 設計概要「インライン編集の発見性」セクションに従い、視覚ヒント / 確定/キャンセル操作 / フィードバック / サーバーエラー時の挙動を本タスクで確定し、以降のインライン編集タスク (V3 / V13) はそれを踏襲する
