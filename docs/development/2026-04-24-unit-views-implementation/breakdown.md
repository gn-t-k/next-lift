...
- 葉タスク: 82件
- 未展開ノード: 0件
- 次のアクション: `#1 基盤整備` から実装開始（`/tdd` スキルで各タスクに着手）
...
2. **ビュー層** (`src/view/`)
   ...
   - 他ビューから使いたくなったら **直接 import してよい**（`_shared/` などへの移動を必須ルールにはしない）
   - 配置を変えたくなるタイミング（以下のいずれかを感じたら検討。ルール化はしない）:
     - 現在の配置場所の名前が所有権を誤って示唆している
     - 配置先のビューが削除される可能性が出てきた
   - 判別基準:
     1. V1〜V15 のどれかに対応する → `src/view/{ビュー名}/index.tsx`
     2. 特定ビュー専用のサブコンポーネント → `src/view/{ビュー名}/` 配下にコロケーション
     3. ドメイン用語を含まない汎用部品 → `src/primitive/`
...
- プリミティブ層のポリシー（Intent UI の扱い、React Aria を真のベースとする方針）
- 2層 + コロケーション構造（プリミティブ / ビュー + 抽出オンデマンド）
...
- [ ] 1-1. プリミティブ層ポリシーのADR作成
  - 説明: Intent UI を起点テンプレート扱いし、React Aria Components を真のベースとする方針をADR化。併せて2層+コロケーション構造と抽出ルールも記述
  - 完了条件: `docs/architecture-decision-record/024-*.md` が作成され、overview.md に追加。関連ADR（014, 015, 023）への相互リンクあり
...
- [ ] 1-4. `packages/react-components/CLAUDE.md` を更新
  - 説明: `src/primitive/` と `src/view/` の配置基準、抽出オンデマンドルール、`src/view/_shared/` の引き上げ手順を追記
...
- [ ] 5-3-2. propsインターフェース + 通常状態ストーリー
  - 説明: V11簡易版として実装。V11の `TrendChart` を再利用するか判断し、再利用する場合は `src/view/_shared/` に引き上げ
  - 完了条件: ストーリーが表示
