...
- 現在のフェーズ: 5/5 完了
- 葉タスク: ~121件（2-3 配下に旧 2-4 (Day 内容) を統合し 22 タスクに再分解、Drawer 2-3-6.5 を追加。実装着手時に各セクションで詳細分解）。完了済 (24件): 1-1, 1-2, 1-3, 1-4, 2-1-1, 2-1-2, 2-1-3, 2-1-4, 2-1-5, 2-1-6, 2-1-7, 2-2-1, 2-3-1, 2-3-2, 2-3-3, 2-3-4, 2-3-5, 2-3-6, 2-3-6.5, 2-3-7, 2-3-8, 2-3-9, 2-3-10, 2-3-11
- 未展開ノード: 0件
- 次のアクション: 2-3-12 種目計画の追加・削除 + ストーリー（ExerciseSelector の統合と ProgramDetail / ExercisePlanSection への props 伝搬も併せて対応）
...
- [ ] 2-3-12. 種目計画の追加・削除 + ストーリー
  ...
  - **進捗 (2026-05-20)**: 追加・削除 API と ExercisePlanSection / ProgramDetail への伝搬、`CreateExercisePlanCard` の新設、種目計画ゼロ件の空状態、追加/削除の play test ストーリーは実装完了 (設計判断 #71)。残作業: ExerciseSelector の ExercisePlanRow 統合 (`availableExercises` / `onSelectExercise` / `onCreateExercise` の受け渡し)、追加直後の picker フォーカス展開の挙動確定。設計判断 #59-D「種目計画追加時の初期構造リセット」は設計判断 #68 で Empty 行廃止により `setPlans: []` に単純化（consumer 責務、設計判断 #71）。Issue #776 の「種目計画追加直後の transient state」は #68 + #70 で構造的に解消済み
  - **設計判断 #71 更新 (2026-05-20)**: 当初は削除を Menu (`EllipsisVerticalIcon` + Menu「種目計画を削除」) 経由としていたが、項目 1 個のメニューを開く操作が冗長なため、SetPlanRow と同じく直接 `TrashIcon` ボタンに変更。階層差で UI を分ける案は撤回し、削除は SetPlan / ExercisePlan で統一。誤タップによる削除リスクは別 breakdown (undo/redo) で取り消し可能性を担保する前提
