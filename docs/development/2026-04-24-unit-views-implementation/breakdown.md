...
- 現在のフェーズ: 5/5 完了
- 葉タスク: 87件（うち 1-1, 1-2, 1-3, 1-4, 2-1-1, 2-1-2, 2-1-3, 2-1-4 完了）
- 未展開ノード: 0件
- 次のアクション: V1 プログラム一覧のローディング状態ストーリー（タスク 2-1-5）から再開
...
- [x] 1-2. `src/ui/` → `src/primitive/` リネーム
...
- [x] 1-3. `src/view/` ディレクトリと `index.ts` を新規作成
...
- [x] 2-1-2. 空状態ストーリーと表示
  ...
  - **NOTE-3の確定方針**: NOTE-3 の「初回はサンプルコンテンツで触って発見させる」はアプリ層の責務（サインアップ時のサンプルプログラム種付け等）とし、`react-components` はピュアプレゼンテーショナル層として「初回」と「空になった」を区別しない。V1 は単一の空状態（構造とラベルで導く）を持つ。新規作成 CTA はタスク 2-1-3 のヘッダーボタンに分離するため、空状態の中には CTA を含めない（タイトル + 説明文のみ）
- [x] 2-1-3. 新規作成リンクの配置 + ストーリー
  ...
  - **配置の確定**: ヘッダー右側に固定配置（FAB ではなくヘッダー）。理由: スクロールから独立した位置で常に視認・到達できる。FAB はリストとオーバーラップして空状態の構造的ヒントを隠すリスクがある。`buttonStyles({ intent: "primary", size: "sm" })` を Link に適用、`PlusIcon`（`@heroicons/react/24/solid`）+「新規作成」ラベル。既存全ストーリー（`MultiplePrograms` / `EmptyPrograms` / `LongProgramName`）でヘッダーに表示される
- [x] 2-1-4. エラー状態ストーリー
  - 説明: 既存 ErrorAlert を差し込んだエラー状態
  - 完了条件: エラー状態ストーリーが表示
  - **実装方針**: `error?: ReactNode` props を追加し、提供時に `ErrorAlert` でメッセージを表示する。エラーは空状態より優先で表示し、ヘッダーは保持（V14への遷移は引き続き可能）。リトライは consumer 責務（presentational 層に閉じない）
- [ ] 2-1-5. ローディング状態ストーリー
...
- [ ] 2-2-1. TextField プリミティブを追加
  - 説明: `pnpm ui:fetch @intentui/text-field` で取得後、意見を剥がして `src/primitive/text-field.tsx` に配置
...
- [ ] 2-3-1. Tabs プリミティブを追加
  - 説明: `pnpm ui:fetch @intentui/tabs` で取得、意見を剥がして `src/primitive/tabs.tsx` に配置
...
- [ ] 2-3-6. Menu/MenuTrigger プリミティブを追加
  - 説明: `pnpm ui:fetch @intentui/menu` で取得後、意見を剥がして `src/primitive/menu.tsx` に配置（ADR-024に従う）
...
- [ ] 2-4-1. NumberField プリミティブを追加
  - 説明: `pnpm ui:fetch @intentui/number-field` で取得、意見を剥がして `src/primitive/number-field.tsx` に配置
...
- [ ] 3-1-8. V6完了時のトースト通知表示 + ストーリー
  - 説明: Toast プリミティブを `pnpm ui:fetch` で取得、意見を剥がして `src/primitive/toast.tsx` に配置。「記録しました」+「詳細を確認する」ボタン（V9導線）
