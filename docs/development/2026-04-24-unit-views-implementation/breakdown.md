...
- 現在のフェーズ: 5/5 完了
- 葉タスク: ~121件（2-3 配下に旧 2-4 (Day 内容) を統合し 22 タスクに再分解、Drawer 2-3-6.5 を追加。実装着手時に各セクションで詳細分解）。完了済 (35件): 1-1, 1-2, 1-3, 1-4, 2-1-1, 2-1-2, 2-1-3, 2-1-4, 2-1-5, 2-1-6, 2-1-7, 2-2-1, 2-3-1, 2-3-2, 2-3-3, 2-3-4, 2-3-5, 2-3-6, 2-3-6.5, 2-3-7, 2-3-8, 2-3-9, 2-3-10, 2-3-11, 2-3-12, 2-3-13, 2-3-14, 2-3-15, 2-3-16, 2-3-17, 2-3-18, 2-3-19, 2-3-20, 2-3-21, 2-3-22
- 未展開ノード: 0件
- 次のアクション: 2-6-1 propsインターフェース設計 + 通常状態ストーリー
...
- [x] 2-3-21. ローディング/エラー状態ストーリー
  - 説明: プログラム取得中、取得失敗
  - 完了条件: 2状態のストーリーが表示
  - 実装メモ: `ProgramDetail` 本体へ `isLoading` / `error` props は追加せず、V1 と同じ状態分離パターンで `ProgramDetailLoading` / `ProgramDetailError` を独立 export として追加。アプリ側が fetch 状態で切り替え、V2 本体は保存済みデータ表示に集中する。ローディングはプログラム情報ヘッダー、Day タブ、種目計画カード、SetPlan 行の形を保った skeleton とし、`aria-busy` + sr-only テキストで読み込み中を伝える。エラーは `role="alert"` + `ExclamationTriangleIcon` + 「プログラムを取得できませんでした」の控えめなメッセージ表示にし、詳細 message は任意 props とした。Storybook は `Loading` / `ErrorDefault` / `ErrorWithMessage` を追加
- [x] 2-3-22. レスポンシブ検証ストーリー（Day パネル統合後の再確認）
  - 説明: 2-3-2 で TabScrollArea によるタブ部分の対応は完了済み。Day パネル中身（種目計画・セット計画）追加後の小画面/大画面表示を再検証
  - 完了条件: 375px / 1280px 両 Viewport で崩れない
  - 実装メモ: `ProgramDetail` の統合済み Day パネル（種目計画・セット計画・実施履歴）を `ResponsiveMobile` / `ResponsiveDesktop` ストーリーとして追加。Storybook の viewport 定義に合わせ、mobile は 375px、desktop は 1280px で固定確認できるようにした
