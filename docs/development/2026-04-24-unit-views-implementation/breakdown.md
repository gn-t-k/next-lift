...
- 現在のフェーズ: 5/5 完了
- 葉タスク: 87件（うち 1-1, 1-2, 1-3, 1-4, 2-1-1, 2-1-2, 2-1-3, 2-1-4, 2-1-5, 2-1-6, 2-1-7, 2-2-1 完了）
- 未展開ノード: 0件
- 次のアクション: 2-2-2 V14 フォーム骨格 + 通常状態ストーリー
...
- [x] 2-2-1. TextField プリミティブを追加
  ...
  - **意見の剥がし**: Intent UI 由来の `select-none` `forced-colors` は維持（アクセシビリティ実用品）。`function` キーワード→アロー関数、import パス（`@/lib/primitive` → `../../libs/primitive`）、`tv` で利用可能な render-prop 非対応 className は `cn` へ切替（Label / Description は静的 className のみ）
