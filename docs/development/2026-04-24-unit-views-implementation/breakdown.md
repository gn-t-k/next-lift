...
- [ ] 2-3-12. 種目計画の追加・削除 + ストーリー
  ...
  - **2026-05-13 進捗 (Issue #776 起点)**: 種目計画の追加・削除 API（`onAddExercisePlan(dayId)` / `onDeleteExercisePlan(id)`、初期構造の生成は consumer 責務、設計判断 #65）と UI（`CreateExercisePlanCard` CTA / ExercisePlanRow ヘッダーの操作メニュー） + 種目計画ゼロ件エッジケースのストーリーを実装済み（PR feature/exercise-plan-add-delete）。**残作業**: (1) 2-3-8 引き継ぎの ExerciseSelector を ExercisePlanRow に統合、(2) セット計画ゼロ件エッジケース（`setPlans.length === 0`）の UX（Issue #776 の真の修正、CreateSetPlanRow の 0/0 既定値挙動の置き換え）
