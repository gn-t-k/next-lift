...
- ステータス: **確定**（2026-04-27、ADR-027由来でV6/V13に単位プリセット関連タスク2件を追加）
- 現在のフェーズ: 5/5 完了
- 葉タスク: 87件（うち 1-1, 1-2, 1-3, 1-4, 2-1-1, 2-1-2, 2-1-3, 2-1-4, 2-1-5, 2-1-6, 2-1-7, 2-2-1, 2-2-2 完了）
- 未展開ノード: 0件
- 次のアクション: 2-2-3 V14 バリデーション/送信中/送信失敗状態のストーリー
...
### フォーム系ビューの設計方針 (V14 / 将来の V12 種目登録等)

フォーム系ビュー (V14 プログラム作成、将来の V12 種目登録など) は、`<form>` 要素はビューが持ち、その属性は `formProps?: ComponentProps<"form">` で外注入する設計を採用する。

- `react-components` は Conform / Server Actions / `useFormStatus` などフレームワーク固有のフォームライブラリを **import しない** (依存ゼロ)
- consumer (apps/web) で `useForm` の `form.props` をそのまま `formProps` に流せば Conform 互換
- consumer が Conform を使わない場合も `formProps={{ onSubmit }}` でシンプルに動く
- フィールドの状態 (`defaultValue`, `isInvalid`, `errors`) は `xxxFieldProps` で外注入 (Conform の `defineCustomMetadata` で生やせる `textFieldProps` と互換の形)
- 送信中の `isPending` も外注入 (`useFormStatus` の `pending` をそのまま渡す)

これにより「ビュー = 1 コンポーネント」原則を保ちつつ、フォームライブラリの自由度を確保する。
...
- [x] 2-1-3. 新規作成リンクの配置 + ストーリー
  - 説明: 新規作成ボタン（FAB or ヘッダー）を `createHref` を使った Link として配置。プログラム作成（V14）への遷移リンクであり、Menuプリミティブは不要
...
- [x] 2-1-4. エラー状態ストーリー
  ...
  - **共通シェル**: ヘッダー（`プログラム`）と `CreateProgramCard` は normal/error/loading で共通表示にし、状態遷移時の視覚的安定性を保つ。エラー時もデータ非依存の `CreateProgramCard` は表示し、V14 への導線を維持する。レイアウトは `<ul>` の最上段に `CreateProgramCard` を固定し、エラーバナーは `<ul>` の**下**に配置する（`CreateProgramCard` の縦位置をすべての状態で不変に保つことを優先）。`createHref: string` props を `ProgramListError` にも持たせる
...
- [x] 2-2-2. V14 フォーム骨格 + 通常状態ストーリー
  - 説明: プログラム名入力 + 送信ボタン (キャンセルはアプリ層のナビゲーションに委ねる)
  - 完了条件: 通常状態ストーリーが表示。propsは `{ formProps, nameFieldProps, isPending, submitLabel }` 相当
  - **コンポーネント名**: `ProgramCreateForm` (ファイル `src/views/program-create-form/program-create-form.tsx`)。命名は既存の `Program*` prefix 群 (`ProgramList`, `ProgramListError`, `ProgramListLoading`) と揃え、ドメインオブジェクト中心の export 一覧を維持
  - **実装方針 (F案)**: ビューが `<form>` を持つが、属性は **`formProps?: ComponentProps<"form">` で外注入**。`react-aria-components` の `Form` は使わず、素の `<form {...formProps}>` でラップ。これにより Conform の `form.props` や Server Action の `action` をそのまま流せる。`react-components` は Conform を import しない (依存ゼロ)
  - **キャンセルボタン撤去**: 戻り操作はアプリ層のナビゲーション (戻るボタン、ブラウザバック、スワイプバック) に委ねる。独立ビューであるため「キャンセル」はモーダル的な語彙であり、ナビゲーションUI の責務として整理
  - **送信ボタンラベル**: デフォルト「**プログラムの設計に進む**」(`submitLabel` で上書き可)。理由: (1) `design-principles.md` 「データと構造で語る」に従い、ボタン押下後の動作 (V2 プログラム詳細での Day 構成・種目配置) を予告、(2) 「保存」は画面遷移で構造的に示せるため明文化不要 (`design-principles.md` 「自動保存で「壊れない」」)、(3) 「Day」は V2 で発見させる方針 (`design-principles.md` 「発見の設計」) のためボタンで先出ししない
  - **ページクロムの分離**: V1 と同様、ヘッダー (「プログラムを始める」等の見出し) と `<main>` ラッパーは consumer が `PageSection` + `PageHeading` で組み立てる。フォーム系のため `width` はデフォルト `"narrow"` (max-w-2xl)
  - **将来の昇格候補**: V12 種目登録が独立ビュー化される場合、`ProgramCreateForm` と類似する 1 フィールドフォームになる。ADR-024 の昇格ルール (2件目で判断) に従い、共通化するなら `primitives/single-field-form/` への昇格を検討 (ドメイン用語なしのため primitive 行きが妥当)
  - **ストーリー**: `Default` / `Mobile`(375px) / `Desktop`(1280px) を `program-create-form.stories.tsx` に追加。`formProps` の `onSubmit` は decorator で `event.preventDefault()` を入れて画面遷移を防ぐ (`storybook/test` の `fn()` で発火を検証可能にする)
...
- [ ] 2-3-6. Menu/MenuTrigger プリミティブを追加
  - 説明: `pnpm ui:fetch @intentui/menu` で取得後、意見を剥がして `src/primitives/menu.tsx` に配置（ADR-024に従う）
  - 完了条件: Menu / MenuTrigger が単体でストーリー化されている
- [ ] 2-3-7. プログラム複製アクション + ストーリー
  - 説明: ビューのコンテキストメニューに「複製」を配置。`onDuplicate` コールバック
  - 完了条件: コンテキストメニューから複製が発火するストーリーが表示
- [ ] 2-3-8. プログラム削除アクション + 確認ダイアログ + ストーリー
  - 説明: ビューのコンテキストメニューに「削除…」を配置。確認ダイアログ（既存 Modal を `role="alertdialog"` で利用）経由で `onDelete` を発火
  - 完了条件: 削除メニュー押下→確認ダイアログ表示→確定で `onDelete` 発火するストーリーが表示
- [ ] 2-3-9. V10（計画実績比較）導線のprops + ストーリー
  - 説明: 計画実績確認への遷移コールバック
  - 完了条件: 導線表示のストーリーが表示
- [ ] 2-3-10. ローディング/エラー状態ストーリー
  - 説明: プログラム取得中、取得失敗
  - 完了条件: 2状態のストーリーが表示
- [ ] 2-3-11. レスポンシブ検証ストーリー
...
- [ ] 2-4-1. NumberField プリミティブを追加
  - 説明: `pnpm ui:fetch @intentui/number-field` で取得、意見を剥がして `src/primitives/number-field.tsx` に配置
...
- [ ] 2-5-1. SearchField / ListBox プリミティブを追加
  - 説明: 両プリミティブを `pnpm ui:fetch` で取得、意見を剥がして配置
