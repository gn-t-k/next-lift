...
- ステータス: **確定**（2026-05-02、設計判断 #54〜#60 で 2-3 / 2-4 / 2-5 を再構成、2-6 (V16 プログラム新規作成) / 2-7 (V17 Day新規作成) を新設。同日、旧 2-4 (V3 Day 詳細) を 2-3 配下に統合再分解し 22 タスクに展開。2026-05-03、Drawer primitive を 2-3-6.5 として追加: 設計判断 #57 で「狭画面=Drawer」が確定要件のため、ComboBox とは別 primitive として独立、画面幅切替は view 層責務とする）
- 現在のフェーズ: 5/5 完了
- 葉タスク: ~121件（2-3 配下に旧 2-4 (Day 内容) を統合し 22 タスクに再分解、Drawer 2-3-6.5 を追加。実装着手時に各セクションで詳細分解）。完了済 (21件): 1-1, 1-2, 1-3, 1-4, 2-1-1, 2-1-2, 2-1-3, 2-1-4, 2-1-5, 2-1-6, 2-1-7, 2-2-1, 2-3-1, 2-3-2, 2-3-3, 2-3-4, 2-3-5, 2-3-6, 2-3-6.5, 2-3-7, 2-3-8
- 未展開ノード: 0件
- 次のアクション: 2-3-9 SetPlanRow 通常表示（SetPlanRow 単独のストーリー化。3 パターン + `params: null` の表示自体は 2-3-7 で実装済み）
...
- [x] 2-3-7. V2 Day パネル骨格 (ExercisePlanSection 配置)
  ...
  - **props 型の確定 (Day → ExercisePlan[] → SetPlan[])**:
    ...
    - `SetPlanParams` は判別共用体: `{ pattern: "weight-x-reps"; weight; reps }` / `{ pattern: "weight-x-rpe"; weight; rpe }` / `{ pattern: "reps-x-rpe"; reps; rpe }`（設計判断 #59-A）
...
- [x] 2-3-8. ExercisePlanRow 通常表示
  ...
  - **実装方針**: `program-detail/exercise-selector.tsx` を新設し、ExercisePlanRow から呼び出す。広画面/狭画面の切替は CSS-only（`md:hidden` / `hidden md:block`）で2つの DOM ツリーを分岐させ、view 層の責務で primitives に意見を持ち込まない（設計判断 #57 / 2-3-6.5 と整合）。
    - 広画面 (`md:`): `ComboBox` primitive をそのまま使用。`menuTrigger="focus"` でクリック即展開、`renderEmptyState` で「種目が登録されていません」/「一致する種目がありません」を出し分け
    - 狭画面: `Drawer placement="bottom"` の trigger ボタンを表示。Drawer 内に `TextField` (検索) + 種目リスト (Button) を組み立て、選択時に `onSelect` 発火 + Drawer close + query クリア
  - **props 設計**: `ProgramDetail` に `availableExercises: Array<{ id, name }>` と `onSelectExercise: ({ exercisePlanId, exerciseId }) => void` を追加。`ExercisePlanSection` で `exercisePlan.id` を closure 化し `ExercisePlanRow` には `(exerciseId) => void` 形で渡す
  - **ストーリー追加**: `ExerciseSelectorVisible` / `ExerciseSelectorVisibleMobile` (375px) / `ExerciseSelectorVisibleDesktop` (1280px) の 3 本。既存 `InitialStateAfterCreation` も picker が機能する状態に
