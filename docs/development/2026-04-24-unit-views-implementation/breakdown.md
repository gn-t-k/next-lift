...
- [x] 2-3-18. V10（計画実績比較）導線のprops + ストーリー
  - 説明: 計画実績確認への遷移コールバック
  - 完了条件: 導線表示のストーリーが表示
  - 実装メモ: `ProgramDetail` に `onViewPlanRecordComparison(dayId)` を追加し、選択中 Day のパネル上部に「計画実績を確認」ボタンを表示する。V2 から V10 は Day 単位の計画実績確認として遷移するため、コールバックには対象 Day id のみを渡す。Storybook は `ViewPlanRecordComparisonInvokesCallback` を追加し、初期選択 Day の導線押下で `onViewPlanRecordComparison("d1")` が呼ばれることを play test で検証
