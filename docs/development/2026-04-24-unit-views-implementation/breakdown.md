...
- [x] 2-3-18. V10（計画実績比較）導線のprops + ストーリー
  - 説明: 計画実績確認への遷移コールバック
  - 完了条件: 導線表示のストーリーが表示
  - 実装メモ: `ProgramDetail` の `Day` に実施履歴 `workouts` を追加し、各 Day パネル下部に `WorkoutHistorySection` を表示する。セクション先頭にはプログラム一覧と同じ「新規追加 + 一覧」イディオムで `「{day.label}」を実施する` affordance を置き、押下時に `onStartWorkoutFromDay(dayId)` を呼ぶ。実施履歴は日付降順で並べ、履歴 item 押下時は `onViewWorkoutDetail(workoutId)` を呼ぶ。履歴 item はタブ上部の種目情報を繰り返さず、アプリ側で集計した `planResult`（例: 計画通り / 一部変更 / 未完了）と、必要な場合のみ最大重量・総ボリュームなどの `highlights` を表示する。履歴 0 件時は空文言を出さず、実施 affordance のみ表示する。V10 は docs 上 V2/V9 から到達可能だが、実施履歴 item 自体はワークアウトというオブジェクトを開く導線として扱い、計画実績比較は V9 側の導線で扱う。Storybook は実施開始と履歴 item から V9 を開く play test を追加
