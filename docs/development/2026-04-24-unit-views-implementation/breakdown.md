...
- [x] 2-1-4. エラー状態ストーリー
  ...
  - **共通シェル**: ヘッダー（`プログラム`）と `CreateProgramCard` は normal/error/loading で共通表示にし、状態遷移時の視覚的安定性を保つ。エラー時もデータ非依存の `CreateProgramCard` は表示し、V14 への導線を維持する（レイアウト: エラーバナー → CreateProgramCard。「何が起きたか」→「今できること」の順）。`createHref: string` props を `ProgramListError` にも持たせる
