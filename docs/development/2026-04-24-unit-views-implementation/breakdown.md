...
- 現在のフェーズ: 2/5（プログラム計画フロー）進行中
- 葉タスク: ~121件（2-3 配下に旧 2-4 (Day 内容) を統合し 22 タスクに再分解、Drawer 2-3-6.5 を追加。実装着手時に各セクションで詳細分解）。完了済 (34件): 1-1, 1-2, 1-3, 1-4, 2-1-1, 2-1-2, 2-1-3, 2-1-4, 2-1-5, 2-1-6, 2-1-7, 2-2-1, 2-3-1, 2-3-2, 2-3-3, 2-3-4, 2-3-5, 2-3-6, 2-3-6.5, 2-3-7, 2-3-8, 2-3-9, 2-3-10, 2-3-11, 2-3-12, 2-3-13, 2-3-14, 2-3-15, 2-3-16, 2-3-17, 2-3-18, 2-3-19, 2-3-20, 2-3-21
- 未展開ノード: 0件
- 次のアクション: 2-3-22 レスポンシブ検証ストーリー（Day パネル統合後の再確認）
...
- [x] 2-3-18. V10（計画実績比較）導線のprops + ストーリー
  - 説明: 計画実績確認への遷移コールバック
  - 完了条件: 導線表示のストーリーが表示
  - 実装メモ: `ProgramDetail` の `Day` に実施履歴 `workouts` と `startWorkoutHref` を追加し、各 Day パネル下部に `WorkoutHistorySection` を表示する。セクション先頭にはプログラム一覧と同じ「新規追加 + 一覧」イディオムで `「{day.label}」を実施する` Link を置き、`startWorkoutHref` へ遷移する。実施履歴は日付降順で並べ、履歴 item は `detailHref` を持つ Link として V9 ワークアウト詳細へ遷移する。履歴 item は Day 単位の最大重量・ボリューム・計画達成判定などを表示せず、アプリ側が exercise_logs / set_logs のメモから拾った `memoPreview` がある場合だけ表示する。履歴 0 件時は空文言を出さず、実施 affordance のみ表示する。V10 は docs 上 V2/V9 から到達可能だが、実施履歴 item 自体はワークアウトというオブジェクトを開く導線として扱い、計画実績比較は V9 側の導線で扱う。Storybook は実施開始と履歴 item がリンクになることを play test で検証
- [x] 2-3-19. V13（種目詳細）への遷移 + ストーリー
  - 説明: 種目名タップで V13 へ遷移（旧 2-4-9）
  - 完了条件: 遷移コールバック発火のストーリーが表示
  - 実装メモ: 完了条件は「遷移コールバック発火」と書いていたが、コードベースの導線は一貫して href ベースの `Link` primitive（Day の `detailHref`、2-3-18 の workout `detailHref` 等）を採用しているため、callback ではなく `Link` で実装。`ExercisePlanSection` の `Exercise` 型に `detailHref: string` を追加し、種目計画カードの見出し（種目名）を `Link` でラップして V13 種目詳細へ遷移させる。リンクは `hover:underline` + `focus-visible:ring` の控えめなアフォーダンス（design-principles の「アニメーション控えめ」と整合）。`detailHref` は純プレゼンテーショナル方針どおり consumer（apps/web）が組み立てる。Storybook は `ExerciseNameLinksToDetail` を追加し、種目名が `role="link"` + 正しい href を持つことを play test で検証
- [x] 2-3-20. V15 導線 props + ストーリー（Web 限定）
  - 説明: Day 種目推移への遷移（旧 2-4-10）
  - 完了条件: 導線表示のストーリーが表示
  - 実装メモ: 当初は「Day パネル下部の Day 単位リンク（Day.progressHref + DayProgressLink）」で実装したが、設計を見直し「各種目名の横の推移トグル」に変更。理由: (1) UC_E_4「プログラム設計時に種目の強度・ボリューム推移を表示」は種目単位の確認であり、Day 単位より種目ごとにピンポイントで見る方が設計作業に沿う、(2) 計画⇔推移の往復で画面遷移すると文脈が切れるため、リンク（遷移）ではなく onClick で popover(desktop)/drawer(mobile) を開閉する補助表示にする（NOTE-1「表示形式はオーバーレイ/サイドパネル等」と整合）。V15 本体（推移グラフ）はフェーズ5(5-3)で未実装のため、`ProgramDetail` の `renderExerciseProgress?: (exerciseId: string) => ReactNode` render prop で「推移表示の口」だけ用意し中身は consumer/後続が注入（純プレゼンテーショナル層の責務分離）。種目名横に ChartBarIcon のトグルを置き、既存 ProgramInfoDialog（desktop=Popover/mobile=Drawer）を流用。render prop 未指定時はトグル非表示。Storybook はダミー推移を注入し desktop で「トグル click → dialog 出現 → 中身表示」を play test で検証。V13 種目詳細への Link（exercise.detailHref）は併存。ProgramInfoDialog が 2 件目利用になったため、汎用 primitive への昇格は別タスクに切り出し。

###### 状態とレスポンシブ

- [x] 2-3-21. ローディング/エラー状態ストーリー
  - 説明: プログラム取得中、取得失敗
  - 完了条件: 2状態のストーリーが表示
  - 実装メモ: `ProgramDetailLoading` / `ProgramDetailError` を `program-detail/index.tsx` に併記（V1 ProgramList の状態バリアントと同じ配置。CLAUDE.md「状態バリアントは index.tsx 内に併記」に従う）。ローディングは `skeletonClass` primitive で ProgramDetail の通常レイアウト（見出し・Day タブ・種目計画カード・実施履歴グリッド）を模したスケルトン、`aria-busy` + `aria-live="polite"` の sr-only テキスト。エラーは `ExclamationTriangleIcon`（24/outline）+ 装飾なしバナー（`flex items-start gap-3 p-4`、強い赤を避ける）、`message?: ReactNode` のオプショナル詳細。ストーリーは index.stories.tsx に集約し、loading/error 単体（mobile/desktop ピン留め）に加え、use() + Suspense + ErrorBoundary でローディング→詳細 / ローディング→エラーの実フロー（`FlowLoadingToDetail` / `FlowLoadingToError`）を再現
