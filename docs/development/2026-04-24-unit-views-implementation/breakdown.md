...
- 現在のフェーズ: 5/5 完了
- 葉タスク: ~121件（2-3 配下に旧 2-4 (Day 内容) を統合し 22 タスクに再分解、Drawer 2-3-6.5 を追加。実装着手時に各セクションで詳細分解）。完了済 (26件): 1-1, 1-2, 1-3, 1-4, 2-1-1, 2-1-2, 2-1-3, 2-1-4, 2-1-5, 2-1-6, 2-1-7, 2-2-1, 2-3-1, 2-3-2, 2-3-3, 2-3-4, 2-3-5, 2-3-6, 2-3-6.5, 2-3-7, 2-3-8, 2-3-9, 2-3-10, 2-3-11, 2-3-12, 2-3-13
- 未展開ノード: 0件
- 次のアクション: 2-3-14 Day ラベルインライン編集 + ストーリー
...
- [x] 2-3-13. Day 追加アクション + Day 削除 + ストーリー
  ...
  - **削除ボタン採用パターン**: 全タブに常時 × 表示（VS Code Editor 的）。モバイルでもどのタブからも直接削除できる前提。タブが多くなって視覚ノイズが気になる場合は将来「選択中タブのみ表示」「ホバー時のみ表示」等への切替を検討する余地を残す。確認ダイアログは設計判断 #71 と一貫させて出さない（誤削除は undo/redo で担保する別 breakdown 前提）
  ...
  - **ストーリー追加**: `AddDayFromExistingDays`（「+ Day」押下 → onAddDay 発火 → Day 4 タブが `aria-selected="true"` になることを play test で検証） / `DeleteDayInvokesCallback`（"Day 1: 上半身プッシュを削除" ボタン押下 → `onDeleteDay("d1")` 発火を検証）。StatefulProgramDetail に `handleDeleteDay` / `lastAddedDayId` state を追加し追加→自動選択フローを実体験可能に
