...
- ステータス: **確定**（2026-05-02、設計判断 #54〜#60 で 2-3 / 2-4 / 2-5 を再構成、2-6 (V16 プログラム新規作成) / 2-7 (V17 Day新規作成) を新設）
- 現在のフェーズ: 5/5 完了
- 葉タスク: ~87件（旧 85 件から 2-4 統合・2-5 撤廃・2-6 / 2-7 新設に伴い再分解。実装着手時に各セクションで詳細分解）。完了済 (15件): 1-1, 1-2, 1-3, 1-4, 2-1-1, 2-1-2, 2-1-3, 2-1-4, 2-1-5, 2-1-6, 2-1-7, 2-2-1, 2-3-1, 2-3-2, 2-3-3
- 未展開ノード: 0件
- 次のアクション: 2-3-4 プログラム名/メタ情報のインライン編集（編集モード仕様は設計判断 #60 を反映）
...
- [ ] 2-3-4. プログラム名/メタ情報のインライン編集 + ストーリー
  - 説明: 各フィールドのインライン編集。設計判断 #54 で「新規作成は V16 で扱う」方針に変わったため、本タスクは **既存編集 (UC_A_6) のインライン編集のみ** を扱う (旧説明にあった「name 空状態 (新規作成直後) のプレースホルダーと初回入力フロー」は V16 / V17 のスコープへ移管)
  - 完了条件: 編集モード (鉛筆アイコンタップで開始 → 編集 → Enter / チェックアイコンで確定 / Escape でキャンセル / blur は何もしない) のストーリーが表示
  - **発見性の仕様 (設計判断 #60 を反映)**: 編集モード入る = 鉛筆アイコン明示タップ / 抜ける (確定) = Enter キー or チェックアイコン明示タップ / キャンセル = Escape / blur = 何もしない (モード維持)。breakdown.md 設計概要「インライン編集の発見性」L120-128 の「タップ→input→blur で確定」純インラインパターンは設計判断 #60 で上書き
  - **本タスクでの確定が以降のインライン編集タスク (V13 種目詳細など) のテンプレートになる**
...
##### 2-4. ~~V3 Day詳細~~ (撤廃: 設計判断 #56) → 2-3 (V2 プログラム詳細) に統合

V3 Day詳細は設計判断 #56 で V2 プログラム詳細に統合（プログラム名・詳細・Day（タブ）・種目計画・セット計画を 1 画面で表示）。本セクションのタスク群は 2-3 配下のタスクとして再構成する必要がある（実装着手時に再分解）。以下は旧タスクで、2-3 への統合方針メモを併記する。

- [ ] ~~2-4-1.~~ NumberField プリミティブを追加 → **2-3 配下に移動 (V2 のセット計画値入力で利用)**
- [ ] ~~2-4-2.~~ V3 骨格 + ExercisePlanセクション → **2-3 配下に移動 (V2 の Day タブパネル中身として実装)**
- [ ] ~~2-4-3.~~ 空状態ストーリー (種目ゼロ / セットゼロ) → **設計判断 #58 により新規作成時はアプリ層が初期生成するため、種目ゼロ・セットゼロの空状態は「途中で削除した」エッジケースのみ。2-3 配下に移動**
- [ ] ~~2-4-4.~~ SetPlanRow のインライン数値編集 + パターン切替 → **2-3 配下に移動。設計判断 #59 (パターン切替=セット単位/値全クリア、セット追加=直前継承、微調整ステッパー) を反映**
- [ ] ~~2-4-5.~~ 種目計画・セット計画の削除操作 → **2-3 配下に移動**
- [ ] ~~2-4-6.~~ Dayラベル編集・Day削除のコンテキストメニュー → **2-3 配下に移動**
- [ ] ~~2-4-7.~~ 種目追加アクション → **2-3 配下に移動。設計判断 #57 で V4 撤廃のため、V4 遷移ではなく ComboBox (Popover/Drawer) で完結**
- [ ] ~~2-4-8.~~ Day間兄弟移動 → **既に V2 (2-3-2) でタブ切替により実装済み**
- [ ] ~~2-4-9.~~ V13（種目詳細）への遷移 → **2-3 配下に移動**
- [ ] ~~2-4-10.~~ V15導線 propsとストーリー（Web限定） → **2-3 配下に移動**
- [ ] ~~2-4-11.~~ ローディング/エラー状態ストーリー → **2-3 配下に移動**
- [ ] ~~2-4-12.~~ レスポンシブ検証ストーリー → **既に V2 (2-3-2) で対応済み**

##### 2-5. ~~V4 種目選択~~ (撤廃: 設計判断 #57) → V2 (2-3) 内 ComboBox に集約

V4 種目選択は設計判断 #57 で撤廃し、V2 内の ComboBox（広画面=Popover、狭画面=Drawer）に集約。本セクションのタスク群は 2-3 配下の「種目選択 ComboBox」タスクとして再構成する必要がある（実装着手時に再分解）。

- [ ] ~~2-5-1.~~ SearchField / ListBox プリミティブを追加 → **ComboBox プリミティブを追加に変更し、2-3 配下に移動**
- [ ] ~~2-5-2.~~ V4 骨格 + 通常状態ストーリー → **不要 (ComboBox プリミティブのストーリーで代替)**
- [ ] ~~2-5-3.~~ 空状態/未該当結果ストーリー → **ComboBox プリミティブのストーリーで吸収**
- [ ] ~~2-5-4.~~ 検索フィルター挙動 → **ComboBox プリミティブの内蔵機能**
- [ ] ~~2-5-5.~~ 種目選択コールバック → **2-3 配下に移動 (V2 の種目計画への種目割当)**
- [ ] ~~2-5-6.~~ ローディング/エラー状態ストーリー → **不要 (ComboBox プリミティブのストーリーで吸収)**
- [ ] ~~2-5-7.~~ ページング戦略の確定 → **不要 (ComboBox は検索 + virtualization でスケール)**
