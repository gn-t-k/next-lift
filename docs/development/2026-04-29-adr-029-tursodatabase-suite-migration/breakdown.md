...
- 5.4. ディレクトリ構造の見直し
  - [ ] 5.4.1. `packages/per-user-database/src/features/client/` のリネーム検討
    - 説明: Phase 5 完了時点で `features/client/` 配下は「DB クライアント生成」の枠を超え、proxy 翻訳・マイグレーション適用・ハンドル管理などドライバ層全体に広がる。実装の固まりに合わせてディレクトリ名を見直すか判断する。候補は `driver/` `database-driver/` `adapter/` など。「リネームしない」という結論も妥当
    - 完了条件:
      - 確認: ディレクトリ名と実装内容の整合性を評価した結論が記録されていること（リネームの有無・新名どちらでも可）
