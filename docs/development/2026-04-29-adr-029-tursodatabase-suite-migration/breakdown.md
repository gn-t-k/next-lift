...
- 状態: 確定
- 現在のフェーズ: 5/5（完了）
- 葉タスク: 75件（Phase 1: 8, Phase 2: 8, Phase 2.5: 4, Phase 3: 10, Phase 4: 19, Phase 5: 10, Phase 6: 16）
- 完了済みタスク: 43件（Phase 1: 8, Phase 2: 8, Phase 2.5: 4, Phase 3: 10, Phase 4: 12（コード作業完了 / 実機検証 7 件は別途）, Phase 5: 1）
- 未展開ノード: 0件
- 最終更新: 2026-05-05
...
依存関係: Phase 1 → Phase 2 → Phase 2.5 → Phase 3 → Phase 4 → Phase 5 → Phase 6 の順で実装する。Phase 6 は Phase 2-3 完了後に着手可能。Phase 2.5 はアダプタの所属パッケージ整理で、Phase 3 が production コードからアダプタを使い始める前に必ず完了させる。
...
- 2.1. `mocked-per-user-database.ts` を新アダプタに切替
  - [x] 2.1.1. `mocked-per-user-database.ts` のクライアント生成を新アダプタ経由に置換
  ...
  - [x] 2.1.2. `testing/factories/` 経路の互換確認
...
- 2.2. `mocked-authentication-database.ts` を新アダプタに切替
  - [x] 2.2.1. `mocked-authentication-database.ts` のクライアント生成を新アダプタ経由に置換
  ...
  - [x] 2.2.2. authentication 用マイグレーション適用経路の整備
...
- 2.3. テストセットアップとマイグレーション適用経路の調整
  - [x] 2.3.1. `testing/setup.ts` の旧依存（`@libsql/client`）削除
  ...
  - [x] 2.3.2. パッケージ間 `testing/index.ts` の export 互換維持
...
- 2.4. CI 全テストグリーン確認
  - [x] 2.4.1. `pnpm test` でモノレポ全体が緑であること
    - 説明: per-user-database / authentication / その他 testing/ を経由する全パッケージのテストを実行する
    - 完了条件:
      - テスト: `pnpm test` の終了コード 0、失敗 0
  - [x] 2.4.2. `pnpm type-check` / `pnpm lint` が緑であること
    - 説明: 型エラー・lint エラーが新アダプタ移行後にゼロ件であることを保証する
    - 完了条件:
      - 型: `pnpm type-check` が exit 0
      - 確認: `pnpm lint` が exit 0

### Phase 2.5: アダプタ別パッケージ化

Phase 2 で `authentication → @next-lift/per-user-database/adapter` という不自然な依存方向が発生したため、アダプタを別パッケージ `@next-lift/turso-drizzle-adapter` に切り出す。Phase 3 以降が production コードからアダプタを import し始める前に完了させ、悪い依存方向を固定化させない。

依存: Phase 2

- [x] 2.5.1. `packages/turso-drizzle-adapter/` パッケージの新設
  - 説明: `package.json` (`name: "@next-lift/turso-drizzle-adapter"`) / `tsconfig.json` / `vitest.config.ts` / `biome.jsonc`（必要なら）を作成。`exports` は `./` または `./adapter` で `proxy-execute` 等を露出。`dependencies` に `@tursodatabase/database@<最新>` と `drizzle-orm` を追加し、per-user-database 側からは `@tursodatabase/database` を削除（移管）。`pnpm-workspace.yaml` には自動で含まれる前提
  - 完了条件:
    - 確認: `pnpm-lock.yaml` に新パッケージが workspace として認識されること
    - 確認: 新パッケージ単独で `pnpm --filter @next-lift/turso-drizzle-adapter type-check` が通ること（中身が空でも）

- [x] 2.5.2. アダプタ実装ファイルの移動
  - 説明: 以下を `packages/per-user-database/src/features/client/` から `packages/turso-drizzle-adapter/src/` に移動する:
    - `proxy-execute.ts` + `proxy-execute.test.ts`
    - `proxy-transaction.ts` + `proxy-transaction.test.ts`
    - `create-turso-database-handle.ts` + `create-turso-database-handle.test.ts`
    - `create-drizzle-from-turso-database.ts`（テストは per-user-database スキーマに依存するため per-user-database 側に残置）
    - `apply-migrations.ts`（テストは per-user-database の drizzle フォルダに依存するため per-user-database 側に残置）
    - `adapter.ts`（再エクスポート + `SqliteRunResult` 型）
  - 残置: `create-per-user-database-client.ts`（per-user-database 固有の Phase 3 置換対象）、`adapter-integration.test.ts` / `create-drizzle-from-turso-database.test.ts` / `apply-migrations.test.ts`（per-user-database スキーマ依存の統合テスト）
  - 完了条件:
    - 確認: 移動後のファイルパスが `packages/turso-drizzle-adapter/src/` 配下になっていること
    - 確認: 新パッケージの内部 import パスが整合すること

- [x] 2.5.3. per-user-database / authentication の依存と import 切替
  - 説明: 両パッケージの `package.json` から `@next-lift/per-user-database/adapter` 依存（authentication 側）を `@next-lift/turso-drizzle-adapter` に変更。per-user-database 側は新パッケージを `dependencies` に追加。各パッケージのソース内の import を以下のように更新:
    - `@next-lift/per-user-database/adapter` → `@next-lift/turso-drizzle-adapter`（authentication 側）
    - `../features/client/adapter` → `@next-lift/turso-drizzle-adapter`（per-user-database 内部の利用箇所、`database-context.ts` 等）
  - per-user-database の `package.json` の `exports` から `./adapter` エントリを削除
  - 完了条件:
    - 確認: `authentication/package.json` の `dependencies` に `@next-lift/per-user-database` が残っていないこと（テスト経路だけのため必要であればそのまま残置可。判断はこのタスク内で行い記録する）
    - 確認: ソース全体で `@next-lift/per-user-database/adapter` の参照が 0 件
    - 確認: ソース全体で `@next-lift/turso-drizzle-adapter` 経由でアダプタが解決できること

- [x] 2.5.4. 検証
  - 説明: 全パッケージの test / type-check / lint を実行して既存 Phase 2 までと同じ結果（緑）になることを確認
  - 完了条件:
    - 確認: `pnpm test`（worktree 環境では env 由来の baseline 失敗のみ）
    - 確認: `pnpm type-check` が緑（per-user-database / authentication / turso-drizzle-adapter 全て）
    - 確認: `pnpm lint` が緑
...
- 3.1. `@tursodatabase/serverless` のインストールとアダプタ拡張
  - [x] 3.1.1. `@tursodatabase/serverless` を関係パッケージに追加
  ...
  - [x] 3.1.2. アダプタを `@tursodatabase/serverless/compat` 入力に対応させる
...
- 3.2. `create-per-user-database-client.ts` の置換
  - [x] 3.2.1. `@libsql/client` import を `@tursodatabase/serverless/compat` に差し替え
...
- 3.3. authentication パッケージの DB アクセス経路置換
  - [x] 3.3.1. `helpers/get-database.ts` の置換
  ...
  - [x] 3.3.2. `development/apply-auth-migration.ts` の置換
...
- 3.4. authentication の dev CLI（create / destroy / reset）置換
  - [x] 3.4.1. `development/cli/create.ts` の DB 接続を新クライアントに置換
  ...
  - [x] 3.4.2. `development/cli/destroy.ts` / `reset.ts` を同様に置換
...
- 3.5. Vercel preview / ローカル Web 動作確認
  - [x] 3.5.1. ローカル `pnpm dev` で Web から Per-User DB / Auth DB アクセスが成功すること
  ...
  - [x] 3.5.2. Vercel preview デプロイで同等の動作確認
...
- 4.1. `@tursodatabase/sync-react-native` のインストールと実機検証スパイク
  - [x] 4.1.1. `@tursodatabase/sync-react-native` を `apps/ios` に追加
...
- 4.2. アダプタを sync-react-native ドライバ用に拡張
  - [x] 4.2.1. アダプタの sync-react-native 用接続経路を実装
...
- 4.3. `apps/ios/src/lib/database.ts` の置換
  - [x] 4.3.1. `openSync()` ＋ `libsqlSyncInterval` 経路を `Database.connect` ＋ アダプタ経由に置換
...
- 4.4. `database-context.tsx` の sync インタフェース変更（`sync()` → `pull()` / `push()`）
  - [x] 4.4.1. `useDatabase()` が返す API を `{ db, pull, push }` に変更
...
- 4.6. ライフサイクルイベント（`AppState` / `useFocusEffect`）での `pull()` 呼び出し追加
  - [x] 4.6.1. `AppState` の `'active'` 遷移で `pull()` を呼ぶフックの追加
  ...
  - [x] 4.6.2. `useFocusEffect` で画面 focus 時に `pull()` を呼ぶ
    - 説明: 画面遷移で復帰したとき、その画面の表示前に `pull()` を呼ぶ。`home-screen.tsx` を最低 1 箇所、ほかの画面は同じパターンを横展開
    - 完了条件:
      - 確認: 実機で他画面 → ホーム画面遷移時に `pull()` が走ること
    - 注記: 現状の `apps/ios` は `react-navigation` / `expo-router` を使用しておらず、`App.tsx` が `useSession` の状態に応じて `SignInScreen` / `DatabaseProvider+HomeScreen` をトップレベル切替する単一画面構成。`useFocusEffect` の発火対象となる画面遷移が存在しないため、本タスクは現アーキテクチャでは N/A として扱う。ナビゲーションが導入された段階で追加実装する
  - [x] 4.6.3. `libsqlSyncInterval` ベースの時間トリガを完全撤去
...
- 4.7. マイグレーション・PRAGMA の実機動作確認
  ...
  - [x] 4.7.2. 旧 DB ファイルを破棄し新規ファイル＋初回 pull で起動する経路の実装
    - 結論: 不要として却下（実装しない）
    - 理由:
      - アプリ未公開のため厳密な互換性は不要（旧データの維持を放棄して良い）
      - 新ファイル名 `per-user.db` は旧ファイル `per-user.sqlite` と異なるパスのため共存しても干渉しない（新規作成 → 初回 pull で Hosted から復元される経路は自動成立）
      - Phase 5 で `@op-engineering/op-sqlite` 依存を撤去した時点で旧ファイルへの参照はソース上ゼロになる
      - 開発端末は 1 人の開発者のみで、必要ならアプリ再インストールで掃除可能
      - `expo-file-system` 等の追加依存導入コストが見合わない
...
- 5.4. ディレクトリ構造の見直し
  - [ ] 5.4.1. `packages/per-user-database/src/features/client/` のリネーム / 構造化検討
    - 説明: Phase 5 完了時点で `features/client/` 配下は「DB クライアント生成」の枠を超え、proxy 翻訳・マイグレーション適用・ハンドル管理などドライバ層全体に広がる。ディレクトリ名と構造を実装の固まりに合わせて見直すか判断する。判断観点:
      - **実装内容との整合**: ディレクトリ名（`client/` のままか `driver/` `adapter/` 等にリネームか）が中身と一致するか
      - **driver 別のグループ化**: in-memory / serverless / sync-react-native の3 driver 系ファイルがフラットに並んでいる状態を継続するか、driver 別サブディレクトリ（例: `proxy/` `turso-database/` `turso-serverless/` `turso-sync/`）に分けるか
      - **捨てやすさ**: Drizzle 公式が `@tursodatabase/*` 用ドライバを提供したとき、Phase 1 で作ったアダプタ群（proxy-execute / proxy-transaction / create-drizzle-from-* / apply-migrations 等）をディレクトリ単位で削除できる構造になっているか。アダプタ部分と呼び出し側を明確に分離しておくと、剥がすときの影響範囲が局所化される
    - 「リネーム / 構造化しない（現状維持）」も結論として妥当
    - 完了条件:
      - 確認: ディレクトリ構造の評価結果（変更の有無・新構造）が記録されていること
...
### ラウンド3: Phase 2 実装中の判断（自動判断）

- 判断: Phase 1 アダプタの `applyMigrations` / `createDrizzleFromTursoDatabase` を schema / migrationsFolder 受け取りに汎用化し、authentication 側からも再利用可能にした（Phase 1 のコードは per-user-database 専用ハードコードだった）
- 判断: アダプタ群を `@next-lift/per-user-database/adapter` として外部公開。Phase 5.4.1 でディレクトリ構造を再評価する前提のため、暫定的に per-user-database 配下に置く
- 判断: Phase 1 で見落とされていた `proxyExecute` の run 戻り値（rowsAffected / lastInsertRowid）抜けを Phase 2 のスコープ内で fix（authentication の `updateToken` 等で実害が出るため）
- 判断: 本番経路の context 型を `LibSQLDatabase<typeof schema>` から driver 非依存の `BaseSQLiteDatabase<'async', SqliteRunResult, typeof schema>` に抽象化（`SqliteRunResult` は libsql `ResultSet` と sqlite-proxy `SqliteRemoteResult` の両方が assignable な最小契約。利用側で参照される `rowsAffected` のみを持つ）。これにより mock 側のキャストが不要になり、Phase 3 で本番ドライバを切り替えた際の型変更も不要になる
- 判断: 当初は mock 側を `as unknown as LibSQLDatabase<typeof schema>` で寄せる案も検討したが、安全契約（`columns` / `lastInsertRowid` 等を読まない前提）が型に表れず Phase 3 への申し送りに依存するため、抽象化の側に倒した
- 判断: `PRAGMA foreign_key_list` は `@tursodatabase/database` で未対応のため、`per-user-database-on-user-deletion.test.ts` の FK 検証は `sqlite_master.sql` の正規表現マッチに変更
- 既知の Phase 2 範囲外の baseline 失敗（PR には含めず別 issue として整理）:
  - authentication: `remove-credentials.test.ts` / `development/{create,destroy,reset}-auth-database.test.ts` の 4 件は `mockPrivateEnv` 抜けで env 検証エラー（CI で `pnpm test` を実行していないため見逃されていた）
  - apps/web: Next.js 16 の `PageProps` が `.next/types` 未生成のため見つからない（Vercel env 取得が前提）
- 補足: 上記 worktree でのテスト失敗は worktree に `.env` を持っていないことが原因。メインの worktree では同じテストが緑になる想定

### ラウンド4: アダプタの所属パッケージ

- Q: Phase 2 で authentication が `@next-lift/per-user-database/adapter` を import する形になっているが、依存方向が不自然ではないか
- A: 自然な形は `(authentication, per-user-database) → 共通アダプタ`。アダプタを別パッケージに切り出すべき
- 判断: Phase 2.5 として「アダプタを `@next-lift/turso-drizzle-adapter` パッケージに切り出す」タスクを追加。Phase 3 が production コードからアダプタを使い始める前に完了させる必要があるため、Phase 2.5 を Phase 3 の前段に置く
- パッケージ名: `@next-lift/turso-drizzle-adapter`
  - 候補比較:
    - `@next-lift/turso-adapter`: 短いが「何にアダプトするか」が省略
    - `@next-lift/turso-drizzle-adapter`: 説明的で「Turso と drizzle のアダプタ」が一目で分かる ← 採用
    - `@next-lift/turso-drizzle`: adapter という暫定的・変換層的ニュアンスが消える
  - 既存の `@next-lift/turso`（Platform API）と並ぶドメインプレフィックス + 役割語の構成
- 判断: 実装は次セッションで実施（本セッションはコンテキスト消費が大きいため、breakdown へのタスク記録のみで完了とする）

### ラウンド5: Phase 2.5 実装中の判断（自動判断）

- 判断: `authentication/package.json` の `@next-lift/per-user-database` 依存を完全に削除した（grep の結果、authentication 側は adapter 経由でのみ per-user-database に依存しており、Phase 2.5 の切替後はゼロ参照になったため）
- 判断: `authentication/package.json` の `@tursodatabase/database` 直接依存も削除した（authentication ソースから直接 import している箇所が無く、adapter 経由でのみ利用していたため。同 dep は新パッケージ `@next-lift/turso-drizzle-adapter` に移管済み）
- 判断: `per-user-database/package.json` からも `@tursodatabase/database` を削除（同上、adapter 経由でのみ利用していたため）
- 判断: 新パッケージの `exports` は `./adapter` ではなく root の `.` に集約（既存 `adapter.ts` を `src/index.ts` にリネームし、利用側は `import { ... } from "@next-lift/turso-drizzle-adapter"` で完結）
- 判断: ファイル移動は `git mv` で履歴を保持（`adapter.ts` → `index.ts` を含む 9 ファイル）
- 検証結果: turso-drizzle-adapter 単体テスト 13 件すべて緑、per-user-database / authentication / その他パッケージのテストも Phase 2 完了時点と同一の baseline 状態（worktree 環境では env 由来の既知失敗のみ）

### ラウンド6: Phase 3 実装中の判断（自動判断）

- 判断: turso-drizzle-adapter に共通 `SqliteExecutor` interface を導入し、`createDatabaseExecutor` / `createServerlessExecutor` の 2 wrapper でドライバ差を吸収。`proxyExecute` / `proxyTransaction` を executor 引数に変更。Phase 4 で sync-react-native 用 wrapper を追加する際も同じ構造を踏襲できるよう一般化
- 判断: serverless 用ファサードとして `createDrizzleFromTursoServerless` / `applyMigrationsToTursoServerless` / `createTursoServerlessClient` を新設。`SqliteRunResult` 等の共通契約は据え置き
- 判断: `createPerUserDatabaseClient` の戻り値を drizzle インスタンス単体から `{ db, client }` に変更。理由: マイグレーション経路（`applyMigrationsToTursoServerless`）が compat `Client` を直接要求するため、上位で client を取り回せる必要がある。シグネチャ変更だが、Phase 2 ラウンド3 と同じ判断軸（必要な抽象化）
- 判断: `migrateDatabase` の引数を `LibSQLDatabase<schema>` から compat `Client` に変更し、内部で `applyMigrationsToTursoServerless` を呼ぶ。`migrationsFolder` 解決は既存通り内包
- 判断: authentication の `createDatabaseClient` は同期 API を維持し、PRAGMA は fire-and-forget で発行（`.catch()` でエラーログ）。理由: serverless `Connection` は execLock により execute が直列化されるため、await しなくても後続クエリより先に PRAGMA が確実に実行される。`createAuth` 全体を async 化すると `apps/web/auth.ts` 等への影響が広範になるため、副作用を最小化した
- 判断: Phase 3.4.1 / 3.4.2 の CLI ファイル本体は `@libsql/client` への直接依存を持たず、`apply-auth-migration.ts`（3.3.2 で置換済み）と `@next-lift/turso/*`（変更不要）経由で構成されているため、Phase 3.3.2 完了で実質的にコード変更は完結。CLI 動作の手動確認は別途必要
- 判断: `runInPerUserDatabaseScope.test.ts` は元々 `:memory:` URL で実 client を生成する経路だったが、compat client は HTTP プロトコル前提で `:memory:` を受け付けない。`create-per-user-database-client.mock.ts` を新設し、テストでは `createPerUserDatabaseClient` 自体を spy する形に変更
- 判断: FK 動作確認テスト（3.2.2）は新クライアント（serverless/compat）の :memory: が動かないため、database executor 経路（`createDrizzleFromTursoDatabase` + `:memory:`）で `programs ↔ days` の FK 違反検出を検証。実機 / Hosted DB 上での FK 動作は 3.5.1 で確認
- 検証結果: turso-drizzle-adapter 19/19・per-user-database 25/25 緑、各 type-check / lint 緑。authentication test は env 由来の baseline 4 件失敗のみ（Phase 2.5 ラウンド3 と同じ）。apps/web type-check は `.next/types` 未生成の baseline 失敗のみ
- 残タスク: 3.5.1（ローカル `pnpm dev` 実機確認）/ 3.5.2（Vercel preview 確認）はユーザー手動確認待ち

### ラウンド7: PR push 後の CI fix と命名整理

- 事象: PR push 後の CI で apps/web の Next.js (Turbopack) ビルドが `non-ecmascript placeable asset` エラーで失敗。`turso-drizzle-adapter` の root エントリが `createTursoDatabaseHandle`（`@tursodatabase/database` のネイティブ `.node` バインディング依存）を含んでいたため、Web bundle 解析で `@tursodatabase/database` まで辿り着いて停止
- 一次対応: root に database 系を残しつつ、`./database` サブパスを追加して利用側（テスト / モック）の import を切替（fix コミット 56c708b）。本番経路は root 据え置きで、Web 用の serverless 系のみが解析される構造に
- 二次整理（ドライバ軸への分割）: 一次対応では「root = serverless 用」が暗黙だった。Phase 4 で `@tursodatabase/sync-react-native` を追加する際にも対称な構造になるよう、root を共通型 (`SqliteRunResult`) のみとし、`./serverless` / `./database` の 2 サブパスへ整理。本番経路 4 ファイルの import を `@next-lift/turso-drizzle-adapter` → `@next-lift/turso-drizzle-adapter/serverless` に切替
- 命名軸の判断: 環境軸（`/web` `/node`）とドライバ軸（`/serverless` `/database`）を比較し、(1) このパッケージの責務がドライバ橋渡しであること、(2) 依存パッケージ名と一致して読みやすいこと、(3) 「Node のサーバ側からも serverless を使う」等のケースで環境名が違和感を生むこと、を理由にドライバ軸を採用
- src 配下のディレクトリ整理: 公開エントリをドライバ軸に揃えたあと、src/ 直下のファイル本体もフラットだったため `drivers/{serverless,database}/` に集約。冗長な接頭辞 (`turso-`, `from-turso-`) をディレクトリ名で表現してファイル名を簡潔化。root エントリは集約役の `index.ts` ではなく中身そのものを表す `sqlite-run-result.ts` を直接指す形にし、コーディング規約「ファイル名と export 名を揃える」に沿わせた

### ラウンド8: Phase 3.5 動作確認

- 確認方法: Vercel preview デプロイで Google ログイン → `/api/per-user-database/credentials` のブラウザアクセスで 200 + JSON 取得 → Turso ダッシュボードで Per-User DB のテーブル一覧確認
- 確認結果:
  - ログイン成功（Auth DB への接続 / 書き込みが動作）
  - credentials API が 200 を返却（Phase 3.3.1 の `get-database.ts` 経路が動作）
  - Per-User DB が作成され、全 4 件のマイグレーションが適用済み（`__drizzle_migrations` 4 件、`exercises.weight_step` カラム存在）。Phase 3.2.1 の `create-per-user-database-client.ts` + `applyMigrationsToTursoServerless` 経路が動作
- マイグレーション件数の差分: ブランチには `0000`〜`0002` の 3 ファイルしかないが、preview DB には 4 件記録されている。これは preview DB が main 由来の `0003_powerful_aqueduct.sql`（exercises.weight_step 追加、commit 70e27e0）まで適用された状態で初期化されたため。Phase 3 PR の動作確認には影響なし
- 判断: 3.5.1（ローカル）/ 3.5.2（preview）はいずれも実体は同じコード経路のため、preview での確認をもって両方完了とする

### ラウンド9: Phase 4 コード作業完了と 4.7.2 の却下判断

- 完了範囲: Phase 4 のコード作業（4.1.1, 4.2.1, 4.3.1, 4.3.2, 4.4.1, 4.4.2, 4.5.1, 4.5.2, 4.6.1, 4.6.3）すべて完了。残るのは実機検証 7 件（4.1.2, 4.1.3, 4.1.4, 4.2.2, 4.7.1, 4.8.1, 4.8.2）と N/A 1 件（4.6.2）
- 4.6.2（useFocusEffect）の N/A 判断: `apps/ios` は `react-navigation` / `expo-router` を使わず、`App.tsx` がトップレベルで `useSession` の状態に応じて `SignInScreen` / `DatabaseProvider+HomeScreen` を切替する単一画面構成のため、画面 focus イベントが発生しない。ナビゲーション導入時に追加実装する
- 4.7.2（旧 DB ファイル削除経路）の却下判断:
  - Q: 旧 `per-user.sqlite`（op-sqlite 由来）のファイル削除コードを書くか？
  - A: 不要として却下
  - 理由:
    - アプリ未公開のため厳密な互換性は不要
    - 新ファイル名 `per-user.db` は旧 `per-user.sqlite` と異なるパスのため共存しても干渉しない（新規作成 → 初回 pull で Hosted から復元の経路は自動成立）
    - Phase 5 で op-sqlite 依存撤去後はソース上の参照ゼロ
    - 開発端末は 1 人の開発者のみで、必要ならアプリ再インストールで掃除可能
    - `expo-file-system` 等の追加依存導入コストが見合わない
- アダプタ実装中の判断:
  - `drizzle-orm/sqlite-proxy/migrator` は内部で `node:fs` / `node:crypto` を使うため React Native では動かない。`drizzle-orm/expo-sqlite/migrator` は `db: ExpoSQLiteDatabase<TSchema>` 型に縛られて sqlite-proxy 由来の drizzle インスタンスに渡せない。両方の制約を回避するため、`SQLiteAsyncDialect.migrate` と同等のロジック（`__drizzle_migrations` テーブル作成 → 最後に適用された migration を `created_at` で取得 → 未適用分を `proxyTransaction` 内で `--> statement-breakpoint` で split しつつ実行 → INSERT）を `applyMigrationsToSyncReactNative` 内に直接書き起こした
  - sync-react-native のアダプタ依存は `peerDependencies`（optional）として宣言。`apps/ios` のみが直接 install し、Web/Node 系の per-user-database / authentication からは見えない構造を維持
  - sync-react-native の Statement.all/get は Row オブジェクト（`Record<string, SQLiteValue>`）を返すため、columns 順の配列に変換するロジックを `Object.values` で実装。Row は readRow() / getAllRows() で columnCount 順に挿入されており、ECMAScript の string キー挿入順保証により整合する
