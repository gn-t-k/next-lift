...
- [ ] 1-2. items 計算ロジック (`buildListItems` 相当) を再設計する
...
- [ ] 2-1. selectedKey 経由で選択中表示と自動ハイライトを実装する
...
- [ ] 2-2. create option の disabled 制御 + 文言を実装する
...
- [ ] 3-1. trigger button に selected exercise 名を表示する
...
- [ ] 3-3. create button を常時表示し disabled 条件を実装する
...
- [ ] 4-1. 既存ストーリーの args を新 API に揃える + 新規シナリオを追加する
...
- [ ] 5-1. 通常表示・選択中表示・選択中ハイライトをまとめて検証する play test を書く
...
- [ ] 5-2. 検索フィルター + create option の文言・disabled 挙動を検証する play test を書く
...
- [ ] 6-1. 通常表示・選択中表示・選択中ハイライトをまとめて検証する play test を書く
...
- [ ] 6-2. 検索フィルター + create button の文言・disabled 挙動を検証する play test を書く
  - 説明: Drawer ストーリーで以下を確認:
...
- [ ] 6-3. 0 件登録ケースの play test を書く
  ...
  - 完了条件:
    - テスト (play): 2 シナリオを検証し pass する
- [ ] 6-4. drawer 高さ安定の play test を書く
