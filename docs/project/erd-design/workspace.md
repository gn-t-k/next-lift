...
- 状態: 完了
- 現在のフェーズ: 5/5（完了。2026-04-16に再検証実施）
- テーブル数: 15（コアエンティティ9 + パラメータテーブル3 + リンクテーブル2 + イベントテーブル1）
- 総カラム数: 61（nullable 7、nullable FK 0）
- 最終更新: 2026-04-16
...
- nullable FKを全廃達成
- exercise_plansからexercise_idを削除し、exercise_plan_exercisesリンクテーブルに分離（プレースホルダー枠対応）
- 計画系テーブル（days, exercise_plans, set_plans）にmeta_info text nullableを追加
...
| exercise_plans.exercise_id → exercise_plan_exercisesリンクテーブル | nullable FK → リンクテーブル分離 | exercise_plan_exercisesテーブルに分離 | プレースホルダー枠（種目未確定の計画枠）に対応。workout_day_linksと同パターン。記録側(exercise_logs)は影響なし |
| 計画系テーブル(days, exercise_plans, set_plans)にmeta_info追加 | nullable属性追加 | meta_info text nullable | programs.meta_infoと同パターンで計画の各階層に自由テキスト注釈を持たせる。プレースホルダー枠の説明もここで表現 |
...
**F6: プレースホルダー枠対応 → exercise_plan_exercisesリンクテーブルに分離**

トレーニングプログラム設計時に「スクワット系アクセサリー」のように具体的な種目を指定せず、役割だけ記述するプレースホルダー枠が必要。exercise_plans.exercise_idはNOT NULL FKのため対応できない。workout_day_linksと同パターン（nullable FK → リンクテーブル）で解決する。

- exercise_plansからexercise_idカラムを削除
- exercise_plan_exercises（exercise_plan_id text PK FK, exercise_id text FK）テーブルを新設
- exercise_plan_idがPK: 1つのexercise_planに最大1つのexercise（1:1関係で親のPKをPKとして使う。weight_reps_params等と同パターン）
- 行あり → 具体的な種目が確定している
- 行なし → プレースホルダー枠（種目未確定）
- 記録側（exercise_logs.exercise_id）は影響なし。記録時は必ず具体的な種目を選ぶ

**F7: 計画系テーブルにmeta_info追加**

計画の各階層に自由テキスト注釈を持たせるため、programs.meta_infoと同パターンでmeta_info text nullableを追加。

- days.meta_info: 例「上半身の日」「デロードウィーク」
- exercise_plans.meta_info: 例「スクワット系アクセサリー」「メイン種目」。プレースホルダー枠の説明もここで記述
- set_plans.meta_info: 例「ウォームアップ」「AMRAP」

計画側のmeta_info（設計の意図）と記録側のmemo（実績の補足）は別の関心事。

**フィードバック対応後のnullable状況:**

- nullableカラム: programs.meta_info, days.meta_info, exercise_plans.meta_info, set_plans.meta_info, exercise_logs.memo, set_logs.rpe, set_logs.memo（7カラム）
- nullable FK: なし（全廃達成）
- set_plansのnullable: なし（サブタイプテーブルで解消。meta_infoを除く）
...
| exercises と exercise_plan_exercises/exercise_logs/one_rep_maxes の関係 | 非依存リレーションシップ。exercise_plan_exercisesが交差エンティティ、他は現構造で問題なし | exercisesは独立マスタ。exercise_plansはexerciseなしで存在可能（プレースホルダー枠）→ exercise_plan_exercisesリンクテーブルで表現済み。exercise_logs/one_rep_maxesはexercise_idをNOT NULL FKで持ち、作成時に必ず種目が指定されるためnullable FKの問題は発生しない |
...
| exercises → exercise_plan_exercises | 1種目が複数の計画枠に使われる | 1リンクは1種目に対応 | 1:N | `\|\|--o{`（1対0以上）。exercise_plan_exercises.exercise_id側 |
| exercise_plans → exercise_plan_exercises | 1計画枠に最大1種目 | 1リンクは1計画枠に対応 | 1:0..1 | exercise_plan_exercises.exercise_plan_id PK側。プレースホルダー枠はリンク行なし |
...
| 関係 | Aから見てB | Bから見てA | 判定 | Mermaid記号 |
| --- | --- | --- | --- | --- |
| exercises → exercise_plan_exercises | 1種目が複数の計画枠に使われる | 1リンクは1種目に対応 | 1:N | `\|\|--o{`。未使用の種目が正当 |
...
- 種目(exercises)は計画側(exercise_plan_exercises)と記録側(exercise_logs)の両方から参照されるが、これは種目マスタが2つの異なる1:N関係で参照されているだけであり、N:Mではない
...
| exercises → exercise_plan_exercises | Yesで非依存。種目はマスタとして独立 | Noで依存。リンク行は必ず種目を参照 | 非依存（Aから見て） | exercise_plan_exercises.exercise_id NOT NULL FK。リンク作成時には必ず種目が指定される。exercise_plan_exercises自体がexercise_plansとの交差エンティティ |
| exercise_plans → exercise_plan_exercises | Yesで非依存。exercise_plansはexerciseなしで存在可能（プレースホルダー枠） | Noで依存。リンク行は必ず計画枠を参照 | 非依存 | **フェーズ3でexercise_plan_exercisesリンクテーブルとして分離済み**。プレースホルダー枠はリンク行の不存在で表現 |
...
exercises関連の非依存リレーションシップ（exercises → exercise_plan_exercises, exercise_logs, one_rep_maxes）について:
- exercises は独立マスタであり、exercise_plan_exercises/exercise_logs/one_rep_maxesが0件でも存在しうる（片側が非依存）
- exercise_logs, one_rep_maxesは作成時に必ずexercise_idが指定される（FKがNULLになるタイミングがない）。交差エンティティ不要
- exercise_plan_exercisesはexercise_plansとexercisesの非依存リレーションシップを表現する交差エンティティ。exercise_plansはexerciseなしで存在可能（プレースホルダー枠）→ リンクテーブルで表現済み
...
| exercise_plan_exercises | exercise_plan_id text PK FK | 親のPKを主キーとして使用。1:0..1関係のため独自idは不要。weight_reps_params等と同パターン |
...
## フェーズ5: 最終レビュー（再実行: 2026-04-16）

前回のフェーズ5以降に複数の変更（多重度0..n修正、weight_input_unit追加、exercise_plan_exercisesリンクテーブル追加、meta_info追加、weight_type+weight_value検証結果追記）が入ったため、全体を再検証する。
...
全27ユースケースについて擬似SQLで読み取り/書き込みを検証し、**全てOK**。

前回のフェーズ5からの主な修正点:
- weight_input_unitカラムを含む全ての書き込み/読み取り擬似SQL（UC_A_2, UC_A_4, UC_A_8, UC_B_2, UC_B_3, UC_B_7, UC_C_1, UC_C_2, UC_D_1, UC_D_2, UC_E_1, UC_E_2, UC_E_4）にweight_input_unitを明示
- exercise_plan_exercisesリンクテーブル経由のクエリを確認（UC_A_2, UC_A_4, UC_B_2, UC_C_1, UC_E_2）
- meta_infoカラムの読み書きを擬似SQLに反映（UC_A_3, UC_A_4, UC_A_5, UC_A_6, UC_C_1, UC_E_2）
- set_plansの3パラメータテーブルを正しくLEFT JOINする形式を明示
...
- テーブル数15の内訳:
  - コアエンティティ9テーブル: programs, days, exercise_plans, set_plans, exercises, one_rep_maxes, workouts, exercise_logs, set_logs
  - パラメータテーブル3種: weight_reps_params, weight_rpe_params, reps_rpe_params（set_plansの「2 of 3」nullable排除のための属性分離）
  - リンクテーブル2種: exercise_plan_exercises（プレースホルダー枠対応）, workout_day_links（Dayなしワークアウト対応）
  - イベントテーブル1種: workout_completions（ワークアウト完了イベントの分離）
- パラメータテーブル3種の分割は「2 of 3」nullableを構造的に解決するためであり、要件に直接根拠がある
- exercise_plan_exercises: プレースホルダー枠（種目未確定の計画枠）を表現するための分離。nullable FK排除
- workout_day_links: Dayに紐づかないフリーワークアウトを表現するための分離。nullable FK排除
- workout_completions: ワークアウト完了イベントのUPDATE回避。完了取り消し = 行削除で表現可能
- 「将来必要になるかもしれない」だけで追加したテーブルはない。one_rep_maxesのイベント化（INSERT only）は1RM推移追跡のシナリオに基づくが、現時点の要件（UC_D_1: 1RM登録更新）でも成立する構造
- DBだけで解決しようとせず、導出値（e1RM, %1RM, ボリューム）はアプリ側で算出。バリデーション（多重度の最低1件制約、plan_typeとパラメータテーブルの排他制御、weight_input_unitの値制約）もアプリ側で担保

**結論:** 要件に対して適切な複雑さ。過剰な構造は検出されなかった。

#### 判断基準をパターンとして暗記していないか

- workout_completionsのイベントテーブル分離: 「ワークアウト完了」がドメイン上のイベントであること、UPDATE回避の具体的メリット（完了取り消し = 行削除で表現可能）が理由として記録されている（設計判断#11）
- set_plansのサブタイプテーブル分割: 「2 of 3」パラメータのnullable完全排除という具体的な問題を解決している（設計判断#17）
- workout_day_linksの中間テーブル: nullable FK排除のための分離であり、Dayなしワークアウト（フリートレーニング）を表現する具体的要件がある（設計判断#18）
- exercise_plan_exercisesのリンクテーブル: プレースホルダー枠（種目未確定の計画枠）という具体的要件があり、workout_day_linksと同パターン（設計判断#29）

**結論:** パターンの機械的適用は検出されなかった。各テーブルに「なぜこの構造にしたか」の理由がある。

#### 要件のヒアリング不足はないか

- 多重度の判断: programs→days, exercise_plans→set_plans, workouts→exercise_logs, exercise_logs→set_logsの4箇所は0以上（オプショナル）で維持。ドメインモデルの「0..n」を尊重し、途中保存を許容する方針。「最低1つ」の制約はワークフローの特定ポイント（完了時等）でアプリ側で強制する。この判断は設計判断#23に記録済み
- サブセットの将来拡張: 種目カテゴリ（設計判断#22）について後方互換性を含めた検討が記録されている
- lbs対応: 初期スコープに組み込み済み。weight_input_unitカラムを4テーブルに追加（設計判断#21）
- 導出項目の保持/排除: 全導出項目が逆算可能であることを検証し、ビジネスルール（Epley公式等）を正確に把握したうえで排除を判断（設計判断#9）
...
| チェック項目 | 結果 | 詳細 |
| --- | --- | --- |
| 命名の一貫性 | OK | テーブル名: 全て複数形snake_case（設計判断#6）。カラム名: 全てsnake_case。FK: `xxx_id`形式で統一。タイムスタンプ: ドメインの事実に即した命名（started_at, completed_at, achieved_at, registered_at）。weight_input_unitは4テーブル（set_logs, one_rep_maxes, weight_reps_params, weight_rpe_params）で同一名称 |
| NULLableカラムの3点チェック | OK | nullable属性カラム7件（programs.meta_info, days.meta_info, exercise_plans.meta_info, set_plans.meta_info, exercise_logs.memo, set_logs.rpe, set_logs.memo）全てにフェーズ3で検討記録あり。「入力しなかった」= 該当なしの意味でNULLの用途が明確（設計判断#20, #30） |
| UPDATE/DELETE管理の判断プロセス | OK | フェーズ3で全テーブルについて変更モデル（UPDATE or INSERT only）と削除モデル（物理DELETE）を検討済み（設計判断#11〜#15） |
| 既存スキーマとの一貫性 | OK | 認証DBスキーマ（Better Auth管理）とは独立した命名規則を採用（設計判断#6）。Per-User DBの既存仮スキーマ（testTable）は本格設計で置き換え。snake_case、タイムスタンプms精度の規則は既存コードベースと一致 |
| 孤立テーブル | OK | 全15テーブルがリレーションシップで接続。exercisesは3つのテーブルから参照されるハブ（exercise_plan_exercises, exercise_logs, one_rep_maxes） |
| 循環参照 | OK | 循環参照なし。計画系: programs→days→exercise_plans→set_plans→params。記録系: workouts→exercise_logs→set_logs。マスタ: exercises→exercise_plan_exercises/exercise_logs/one_rep_maxes。リンク: workout_day_links, workout_completions |
| サロゲートキー | OK | 独立テーブル11種にid text PK。子テーブル4種（weight_reps_params, weight_rpe_params, reps_rpe_params, exercise_plan_exercises）は親のPKをPK FKとして使用（設計判断#25） |
| 多重度の明示 | OK | 全14リレーションシップにMermaid記号で多重度を明示。0以上を採用した4箇所（programs→days, exercise_plans→set_plans, workouts→exercise_logs, exercise_logs→set_logs）の理由は設計判断#23に記録 |
| weight_input_unitの配置 | OK | 重量入力を伴う4テーブルに配置済み: set_logs（実績の入力単位）, one_rep_maxes（1RMの入力単位）, weight_reps_params（計画の入力単位）, weight_rpe_params（計画の入力単位）。reps_rpe_paramsは重量入力なしのため不要。全てtext NOT NULL |
| meta_infoの配置 | OK | 計画系4テーブルに配置: programs.meta_info（漸進ルール等）, days.meta_info（Dayの補足）, exercise_plans.meta_info（プレースホルダー枠の説明等）, set_plans.meta_info（ウォームアップ/AMRAP等）。全てtext nullable。記録側はmemoで別の関心事として分離 |
| exercise_plan_exercisesのリレーション | OK | exercise_plan_id PK FK → exercise_plans.id、exercise_id FK → exercises.id。1つのexercise_planに最大1つのexercise（1:0..1）。行なし = プレースホルダー枠。Mermaid ERDの記号 `exercise_plan_exercises \|\|--\|\| exercise_plans` は exercise_plan_exercises側が必ずexercise_plansに属する（NOT NULL FK）ことを表現 |

### 最終ERD（フェーズ4と同一、変更なし）

フェーズ5の再検証でERD構造への変更は不要と判断した。フェーズ4の最終ERDがそのまま最終版となる。
...
- [x] 擬似SQLで読み取り（SELECT）と書き込み（INSERT/UPDATE/DELETE）の両方を検証したか → R/Wの両方を記述。weight_input_unitを含む全操作を明示
- [x] 命名規則が統一されているか → snake_case統一、複数形統一、タイムスタンプは事実に即した命名
- [x] 全フェーズの設計判断とその理由が記録されているか → 設計判断#1〜#30が設計判断サマリーに記録済み
- [x] 最終ERDがMermaid形式で最新の状態を反映しているか → フェーズ4のERDと同一（変更なし）。weight_input_unit、meta_info、exercise_plan_exercisesリンクテーブルが全て反映済み

#### 共通ルールチェックリスト

- [x] 設計判断がpros/consで記述されているか → 全フェーズの設計判断にpros/consまたは理由が記載
- [x] 排除したエンティティ・属性に将来シナリオと後方互換性の検討があるか → 導出値排除（#4, #9）、種目カテゴリ排除（#22）に検討記録あり
- [x] ユーザー確認が必要な判断を「⚠️ 要確認」マークで記録したか → 再検証で新たな「⚠️ 要確認」項目は発生しなかった

#### テーブル設計ルールチェックリスト

- [x] enum型やCHECK制約を使っていないか → plan_type, weight_type, weight_input_unitの区分値はアプリ側定数で管理
...
**1. weight_input_unitの擬似SQL反映:**

前回の擬似SQLではweight_input_unitが以下の箇所で欠落していた:
- UC_A_2（プログラム複製）: パラメータテーブルの読み取り・書き込みにweight_input_unitを明示
- UC_A_4（種目配置）: weight_reps_params, weight_rpe_paramsのINSERTにweight_input_unitを追加
- UC_A_8（種目一覧閲覧）: one_rep_maxesのサブクエリにweight_input_unitを追加
- UC_B_2（セット記録）: set_logsのINSERTにweight_input_unitを追加。種目別デフォルト入力単位の取得クエリを追加
- UC_B_3（記録値修正）: set_logsのUPDATEにweight_input_unitを追加
- UC_B_7（ワークアウト事後修正）: set_logsのUPDATEにweight_input_unitを追加
- UC_C_1（計画実績比較）: 計画側・実績側の両方にweight_input_unitを追加
- UC_C_2（強度・ボリューム推移確認）: set_logsのSELECTにweight_input_unitを追加
- UC_D_1（1RM登録更新）: one_rep_maxesのINSERTにweight_input_unitを追加
- UC_D_2（e1RM採用）: one_rep_maxesのINSERTにweight_input_unitを追加。算出元セットのweight_input_unitを引き継ぐことを明記
- UC_E_1（e1RM自動算出）: set_logsのSELECTにweight_input_unitを追加
- UC_E_2（計画値プリフィル）: パラメータテーブルのLEFT JOINにweight_input_unitを追加
- UC_E_4（推移データ表示）: set_logsのSELECTにweight_input_unitを追加

**2. exercise_plan_exercisesリンクテーブルの検証:**

以下のユースケースでexercise_plan_exercisesが正しく使われていることを確認:
- UC_A_2（プログラム複製）: exercise_plan_exercisesの読み取り・書き込みが含まれている
- UC_A_4（種目配置）: 通常の種目配置ではexercise_plan_exercisesにINSERT、プレースホルダー枠ではINSERT省略
- UC_A_7（プログラム削除）: exercise_plan_exercisesの削除が含まれている
- UC_A_11（種目削除）: exercise_plan_exercisesの関連レコード削除が含まれている
- UC_B_2（セット記録）: プリフィル時にexercise_plan_exercises経由で種目を特定
- UC_C_1（計画実績比較）: exercise_plan_exercisesをLEFT JOINでプレースホルダー枠も含む
- UC_E_2（計画値プリフィル）: exercise_plan_exercisesをLEFT JOINでプレースホルダー枠も含む

**3. meta_infoの配置確認:**

- programs.meta_info: UC_A_5で更新。従来から存在
- days.meta_info: UC_A_3, UC_A_6で更新
- exercise_plans.meta_info: UC_A_4で設定（プレースホルダー枠の説明等）、UC_A_6で更新、UC_C_1で読み取り
- set_plans.meta_info: UC_A_4で設定（ウォームアップ/AMRAP等）、UC_A_6で更新、UC_C_1で読み取り

**4. 多重度0..nの4箇所の検証:**

設計判断#23で0以上（オプショナル）に統一した4箇所:
- programs → days: `||--o{`（1対0以上）。0はプログラム作成途中の状態
- exercise_plans → set_plans: `||--o{`（1対0以上）。0はセット設定前の途中保存
- workouts → exercise_logs: `||--o{`（1対0以上）。0はワークアウト開始直後
- exercise_logs → set_logs: `||--o{`（1対0以上）。0は種目追加直後

全てMermaid ERDの記号が `||--o{` で正しく表現されていることを確認。「最低1つ」の制約はワークフローの特定ポイント（完了時等）でアプリ側で強制する方針。

**5. Mermaid ERDのカラム定義の最新確認:**

全15テーブルのカラム定義をMermaid ERDと照合:
- weight_input_unit: set_logs, one_rep_maxes, weight_reps_params, weight_rpe_paramsに存在 ✓
- meta_info: programs, days, exercise_plans, set_plansに存在（全てnullable） ✓
- exercise_plan_exercises: exercise_plan_id PK FK, exercise_id FK ✓
- set_plans.plan_type: text "NOT NULL" ✓
- reps_rpe_params: weight関連カラムなし（reps + rpeのみ） ✓
...
計画と実績の対比は、同一Dayに対する計画（exercise_plans + exercise_plan_exercises + set_plans）と実績（exercise_logs + set_logs）をexercise_idでマッチングする。exercise_plan_exercises経由でexercise_idを取得する（LEFT JOINでプレースホルダー枠も含む）。同じDayに同じ種目が複数回配置されている場合はdisplay_orderで対応付けする。このマッチングはアプリ側のロジックで行い、SQLは計画データと実績データを個別に取得する。
...
**UC_E_1（e1RM自動算出）のクエリ方針変更:**

前回はORDER BY + LIMIT 1でSQL側で最大e1RMを取得する方針だったが、SQLの計算式に依存する形はアプリ側のe1RM算出ロジックと二重管理になる。全レコードを取得してアプリ側で最大e1RMを算出する方針に変更した。パーソナルユースでデータ量が限定的なため、パフォーマンス上の問題はない。
...
## 2026-04-27 追補：weight_input_unit 再設計（Issue #663）

### 背景

PR #660 のスキーマレビューで設計判断 #21 のオーバーロード問題が露見した。

`weight_reps_params` / `weight_rpe_params` の `weight_value` は `weight_type` によって解釈が変わる:

- `weight_type = "kg"` → `weight_value` は kg値、`weight_input_unit` は入力単位として意味あり
- `weight_type = "percent_1rm"` → `weight_value` は無単位の%値。`weight_input_unit` は NOT NULL で値を持つが意味がない

### 論点と決定（ADR-027）

3つの論点に分けて pros/cons 比較し、3層分離方針を採用。

**論点1: 種目別デフォルト単位の保持位置**
- 採用: `exercises.default_weight_input_unit text NOT NULL DEFAULT 'kg'` カラム追加
- 却下: 独立テーブル化（過剰正規化）/ 動的導出のみ（lbs派ユーザー初回体験劣化）/ ユーザー全体1個（複数ジム要件と矛盾）

**論点2: 計画パラメータの `weight_input_unit`**
- 採用: 完全削除（DROP COLUMN）
- 却下: NULLABLE化（nullable排除方針 #19 と逆行）

**論点3: 実績記録系の `weight_input_unit`**
- 採用: 残す（事実値として）
- 却下: 削除（記録の不可逆性 #12, #14 と矛盾）

### 種目別デフォルト単位の更新タイミング

(1) 種目詳細ビューでの明示編集、(2) セット記録/1RM登録時に異なる単位入力があったらモードレスダイアログで確認して更新。セット記録時の自動上書き（last-write-wins）は副作用が大きいため不採用。

### カラム名の検討

| 候補 | 評価 |
| --- | --- |
| `weight_input_unit`（set_logs と同名） | 関心事は同じだがカラム名で「事実 vs 設定」を区別できず |
| `weight_unit` | 内部kg統一ルールと衝突し誤解を招く |
| `default_unit` | 将来 distance unit が出ると曖昧 |
| **`default_weight_input_unit`** | `default_` で「設定」、`weight_input_unit` で関心事の対応を明示 |

### ユースケースクエリへの影響

13ユースケース中、計画パラメータ参照（UC_A_2 / UC_A_4 / UC_A_6 / UC_C_1 / UC_E_2）から `weight_input_unit` を削除し、必要な箇所では `exercises.default_weight_input_unit` をJOIN参照する形に変更。実績系参照（UC_B_2 / UC_B_3 / UC_B_7 / UC_C_2 / UC_D_1 / UC_D_2 / UC_E_1 / UC_E_4）はそのまま維持。UC_A_8 / UC_A_9 / UC_A_10 で種目編集・登録のクエリに `default_weight_input_unit` を追加。

詳細は ADR-027（`docs/architecture-decision-record/027-weight-input-unit-storage-policy.md`）。
