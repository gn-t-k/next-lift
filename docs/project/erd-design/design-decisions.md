...
| 33 | `exercises` に `weight_step real NOT NULL DEFAULT 2.5 CHECK (weight_step > 0)` を追加 | ui-design設計判断 #59-E でセット計画値の微調整ステッパー（`-2.5kg/+2.5kg/-1回/+1回`）を採用したが、刻み量はバーベル系=2.5kg / ダンベル系=1kg or 0.5kg / ケトルベル=4kg と種目ごとに異なる。Pros(スキーマ保持): 種目ごとに正しい刻み量を反映でき、ユーザビリティが落ちない。種目登録/編集時に調整可能。`default_weight_input_unit`（ADR-027）と同じ「種目ごとの設定」位置づけで一貫。Cons(スキーマ保持): カラム追加とマイグレーションのコスト。Pros(UIハードコード): 実装が単純。Cons(UIハードコード): 全種目で同じ刻みになりユーザビリティ低下、種目別運用ができない。判断: スキーマ保持。DEFAULT は 2.5（バーベル系の標準）。CHECK `> 0` は数値の妥当性検証で区分値 enum とは別の関心事。回数の刻みは ±1 固定で UI ハードコード継続（`reps_step` カラムは追加しない） | post-5 |
