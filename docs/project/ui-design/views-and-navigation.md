...
| V2 | プログラム詳細 | プログラム, Day | UC_A_1, UC_A_3, UC_A_5, UC_A_6 | iOS / Web |
...
※ V14 は設計判断 #53 により撤廃。プログラム新規作成 (UC_A_1) は V2 のインライン編集に統合し、V1 → V2 直接遷移とする。
...
| V2 プログラム詳細 | プログラム名入力 (新規作成時の初回入力) | プログラム | C | インライン編集 |
...
**~~差異3: プログラム作成（V14）後の遷移先~~** → 設計判断 #53 で V14 自体を撤廃したため解消

V14 を独立ビューとして残す前提で「V14 → V2 への自動遷移」を解決策としていたが、設計判断 #53 で V14 自体を撤廃し V1 → V2 直接遷移に統一したため、この差異と解決策ごと無効化。プログラム新規作成 (UC_A_1) は V2 のインライン編集で扱う。
...
※ V14 (プログラム作成) は設計判断 #53 により撤廃。プログラム新規作成 (UC_A_1) は V1 → V2 直接遷移 + V2 のインライン編集に統合。
