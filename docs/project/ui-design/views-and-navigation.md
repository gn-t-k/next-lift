...
| V2 プログラム詳細 | セット追加 | セット計画 | C | 種目計画セクション内のアクションボタン。直前のセットの値とパターンを継承（設計判断 #59） |
| V2 プログラム詳細 | セット計画編集 | セット計画 | U | インライン編集（数値タップ + 微調整ステッパー `-2.5kg/+2.5kg/-1回/+1回`、設計判断 #59） |
| V2 プログラム詳細 | セット計画パターン切替 | セット計画 | U | セット単位でパターン（重量×回数 / 重量×RPE / 回数×RPE）を切替。切替時は値全クリア（設計判断 #59） |
