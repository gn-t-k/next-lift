...
- プログラム編集時は、プログラム詳細（V2）内で Day タブ + Day内容（種目計画・セット計画）を 1 画面で表示（設計判断 #56 で V3 を V2 に統合）
- 振り返り時は、種目詳細（V13）でグラフ表示（V11 への遷移、または埋め込み）
...
| V2 | プログラム詳細 | プログラム, Day, 種目計画, セット計画 | UC_A_3, UC_A_4, UC_A_5, UC_A_6, UC_E_4 | iOS / Web |
...
| V16 | プログラム新規作成 | プログラム | UC_A_1（プログラム名 + 詳細セクション） | iOS / Web |
| V17 | Day新規作成 | Day, 種目計画, セット計画 | UC_A_1（Day構造セクション、プレースホルダー値あり） | iOS / Web |

※ V14 は設計判断 #53 により撤廃。プログラム新規作成 (UC_A_1) は当初 V2 のインライン編集に統合する方針だったが、設計判断 #54 で再判断し、新「プログラム新規作成」(V16) を独立フォームとして導入した（旧 V14 とは別物：複数フィールド + 将来の AI 連携を見越した構造）。
※ V3 Day詳細 は設計判断 #56 により撤廃し、V2 プログラム詳細に統合（プログラム名・詳細・Day（タブ）・種目計画・セット計画を 1 画面で表示）。
※ V4 種目選択 は設計判断 #57 により撤廃し、V2 内の ComboBox（広画面=Popover、狭画面=Drawer）に集約。
※ 新規作成フローは V1 → V16 → V17 → V2 の順（設計判断 #54, #55）。
※ V16 / V17 の入力ビューでは、フィールド単位の「編集モード」概念を持たず、全属性が常時編集状態（フォーム形式）で全体 valid のタイミングで都度保存（設計判断 #60）。

### アクション配置

| 単位ビュー | アクション | 操作対象 | CRUD | 配置方針 |
|-----------|-----------|---------|------|---------|
| V1 プログラム一覧 | 新規作成 | プログラム | C | ビュー内のアクションボタン（FABまたはヘッダー）→ V16 へ遷移 |
| V2 プログラム詳細 | Day追加 | Day | C | ビュー内のアクションボタン |
| V2 プログラム詳細 | メタ情報編集 | プログラム | U | インライン編集（鉛筆アイコン → 編集 → Enter / チェックアイコンで確定、設計判断 #60） |
| V2 プログラム詳細 | プログラム名変更 | プログラム | U | インライン編集（同上） |
| V2 プログラム詳細 | 複製 | プログラム | C | ビューのコンテキストメニュー |
| V2 プログラム詳細 | 削除 | プログラム | D | ビューのコンテキストメニュー |
| V2 プログラム詳細 | 種目追加 | 種目計画 | C | Dayタブ内のアクションボタン（設計判断 #56 で V3 から統合） |
| V2 プログラム詳細 | セット追加 | セット計画 | C | 種目計画セクション内のアクションボタン。直前のセットの値とパターンを継承（設計判断 #59）。別パターンで追加したい場合は追加 UI でパターンを指定（設計判断 #62） |
| V2 プログラム詳細 | セット計画編集 | セット計画 | U | インライン編集（鉛筆アイコンで Popover/Drawer を開き、フィールド commit で都度 auto-save、設計判断 #60/#64）。微調整ステッパー `-2.5kg/+2.5kg/-1回/+1回`（設計判断 #59）。パターン変更はセット削除→追加で行う（設計判断 #62） |
| V2 プログラム詳細 | 種目計画削除 | 種目計画 | D | コンテキストメニューまたはスワイプ |
| V2 プログラム詳細 | セット計画削除 | セット計画 | D | コンテキストメニューまたはスワイプ |
| V2 プログラム詳細 | Day削除 | Day | D | ビューのコンテキストメニュー（Dayタブから） |
| V2 プログラム詳細 | Dayラベル変更 | Day | U | インライン編集（鉛筆/チェックアイコン） |
| V2 プログラム詳細 | 種目選択 | 種目 | U | ComboBox（広画面=Popover、狭画面=Drawer）。検索 + 選択 + 「新規追加」エントリ（設計判断 #57） |
| V16 プログラム新規作成 | プログラム名入力 | プログラム | C | フォーム入力（ふつうのフィールド、編集モード概念なし） |
| V16 プログラム新規作成 | 詳細入力 | プログラム | C | フォーム入力（ふつうのフィールド） |
| V16 プログラム新規作成 | 入力方法選択 | プログラム | C | 「手動で入力」/「AIで生成」ボタン（AI生成は将来追加）→ V17 へ遷移 |
| V17 Day新規作成 | Day名入力 | Day | C | フォーム入力（プレースホルダー「Day 1」） |
| V17 Day新規作成 | 種目選択 | 種目計画 | C | ComboBox（広画面=Popover、狭画面=Drawer） |
| V17 Day新規作成 | セット計画値入力 | セット計画 | C | フォーム入力 + 微調整ステッパー（設計判断 #59） |
| V17 Day新規作成 | Day追加 | Day | C | ビュー内のアクションボタン（複数Day作成時） |
| V17 Day新規作成 | 完了 | Program/Day構造 | C | フォーム全体validで都度保存。完了 → V2 へ遷移 |
...
| ~~V2 プログラム詳細~~ | ~~プログラム名入力 (新規作成時の初回入力)~~ | ~~プログラム~~ | ~~C~~ | ~~インライン編集~~ → 設計判断 #54 で V16 プログラム新規作成に移管 |

※ 削除系アクション（確認ダイアログを伴うモーダルコマンド）は、実装時にコマンド名の末尾に「…」を付与する（例: 「削除…」）。モードレス原則に基づき、別タスク（確認ダイアログ）があることをユーザーに示唆する。

※ プログラムの複製・削除は **V1 プログラム一覧ではなく V2 プログラム詳細のコンテキストメニュー** に集約する。理由: プログラムは設計コストの高いオブジェクトで使用頻度に対する複製・削除頻度が低いため、一覧から直接実行できる必要性が薄い。一覧をクリーンに保ち、誤操作（特に削除）のリスクを下げる。iOS のスワイプ削除パターンも採用しない。
...
- プログラムタブ → V1 → V2 の階層構造（V2 が Day・種目計画・セット計画を統合表示、設計判断 #56）
- 新規作成時は V1 → V16 → V17 → V2 の順（設計判断 #54, #55）
- V2 → V13（種目詳細への横断）、V2 内の ComboBox で種目選択（設計判断 #57）
- ワークアウトタブ → V5 → V6 → V5+V8（完了後トースト通知）のワークアウト記録フロー
- V8 → V9 → V10 のワークアウト履歴・振り返り
- アカウント → V12 → V13 → V11 の種目管理・推移グラフ
- V13 への複数到達経路（V2, V9/V6, V12 から）
...
**~~差異3: プログラム作成（V14）後の遷移先~~** → 設計判断 #53 で V14 自体を撤廃 → 設計判断 #54 で再判断し V16 を導入

設計判断 #53 で V14 を撤廃し V1 → V2 直接遷移としたが、設計判断 #54 で「V2 のインライン編集統合は複数フィールドと将来の AI 連携を見越すと不都合」と再判断し、新「プログラム新規作成」(V16) を独立フォームとして導入。新規作成フローは V1 → V16 → V17 → V2 となる（旧 V14 はプログラム名のみのシンプルフォームで、V16 は複数フィールド + AI 連携を見越した別物）。
...
| 起点 | 兄弟移動先 | 操作 | 目的 |
|------|-----------|------|------|
| プログラム詳細（V2）の Day タブ | 隣接する Day タブ | タブ切り替え/スワイプ | プログラム内のDay間を素早く移動 |

Day間の兄弟移動はPhase 4のフレーム構造（Dayをタブ状に並べる）と整合する（設計判断 #56 で Day詳細を V2 に統合した結果、Day 間の移動は V2 内のタブ切替で完結）。ワークアウト間の兄弟移動は不要と判断した（ワークアウトは時系列で並ぶだけで構造的な親子関係がなく、同じプログラムに属する保証がない。ワークアウト間の比較は計画実績比較（V10）や推移グラフ（V11）で対応可能）。

種目詳細（V13）間の兄弟移動は、種目一覧（V12）からの到達時にはリスト内の前後移動が考えられるが、横断到達（V2やV9から）の場合は文脈が異なるため不自然。種目一覧経由の場合のみ検討の余地があるが、初期スコープでは不要と判断。
...
| 呼び出し元 | 視覚的優先度 | 理由 |
|-----------|------------|------|
| V2: プログラム詳細 | 通常 | 計画中は種目情報の確認が自然な操作（設計判断 #56 で V3 統合） |
...
※ V2 内では Day はタブ切替で表示し、種目選択は ComboBox（広画面=Popover、狭画面=Drawer）で完結する（設計判断 #56, #57）。
...
| V2: プログラム詳細 | V1（既存プログラム選択）、V17（Day新規作成完了） | V1 |
| ~~V3: Day詳細~~ | ~~V2（Day選択）~~ | ~~V2~~ → 設計判断 #56 で V2 に統合（撤廃） |
| ~~V4: 種目選択~~ | ~~V3（種目追加）~~ | ~~V3~~ → 設計判断 #57 で V2 内 ComboBox に統合（撤廃） |
...
| V13: 種目詳細 | V12（選択）、V2（種目名タップ）、V9（種目名タップ）、V6（種目名タップ） | 呼び出し元（V12, V2, V9, V6） |
| V15: Day種目推移 | V2（推移参照、Web限定） | V2 |
| V16: プログラム新規作成 | V1（新しいプログラム） | V1（キャンセル時）/ V17（入力完了時） |
| V17: Day新規作成 | V16（入力完了） | V16（戻る時）/ V2（完了時） |

※ V15はV2に付随する補助表示。表示形式（オーバーレイ、サイドパネル等）はプラットフォーム適合フェーズで確定する。

※ V14 (プログラム作成) は設計判断 #53 により撤廃。設計判断 #54 で再判断し、新「プログラム新規作成」(V16) を独立フォームとして導入（旧 V14 とは別物）。新規作成フローは V1 → V16 → V17 → V2。
