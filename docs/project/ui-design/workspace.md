...
e1RMを概念オブジェクトから導出ビューに変更したことで、ワークアウト削除時の連動削除問題は自動的に解消された。e1RMは保存されず、セット記録から都度算出されるため、ワークアウト（およびそのセット記録）が削除されれば対応するe1RM算出も消える。

#### 1RM削除と%1RMセット計画への影響

1RM削除（未設定に戻す）後、%1RM指定のセット計画はkg換算ができなくなる。

確定方針: セット計画の%1RM指定はそのまま保持し、1RMが未設定の場合はUI上で「1RMが未設定のため計算できません」と表示してユーザーに再設定を促す。また、1RM削除時に%1RM指定のセット計画がある場合は削除確認ダイアログで影響範囲を事前通知する。

- Pros: 計画データを壊さない。%1RMの値自体は有効（1RMを再設定すれば計算可能に戻る）。削除時警告で意図しない操作を防止
- Cons: 警告UIの実装が必要だが、種目削除（設計判断#34）と同じパターンで対応可能

#### 1RM参照先のワークアウト削除時の影響 → Phase 4で確定

1RMが参照するセット記録の親ワークアウトが削除された場合、1RMの参照先が消滅する。参照をnullにして重量値は保持する。ワークアウト削除時に1RM参照先が含まれる場合は警告ダイアログを表示する。
...
##### 1RM参照先のワークアウト削除時の影響 → 確定

1RMが参照するセット記録の親ワークアウトが削除された場合の扱い。

決定: 1RMの重量値は保持し、参照先をnullにする。追加要件として、ワークアウト削除時に1RM参照先が含まれる場合は警告ダイアログを表示する。
...
##### ~~ワークアウト記録中のフルスクリーンモード~~ → 廃止

~~ワークアウト記録中（V6）はボトムタブバーを非表示にし、フルスクリーンで表示する。~~
...
```
セット記録（V6）: 1セットずつ記録
  ← ワークアウト開始（V5）: プログラム・Day選択、計画値プリフィル
    ← ワークアウトタブ（グローバルナビ）
```

ワークアウト完了後:
```
ワークアウト開始+履歴（V5+V8）: トースト通知「記録しました」+「詳細を確認する」ボタン
  ← セット記録（V6）: ワークアウト完了アクション
```

過去のワークアウト閲覧・修正:
```
ワークアウト詳細（V9）: 記録内容の閲覧・事後修正
  ← ワークアウト履歴（V8）: ワークアウトを選択
    ← ワークアウトタブ（グローバルナビ）
```
...
種目詳細（V13）
  ...
  ← ワークアウト詳細（V9）/ セット記録（V6）内の種目記録: 種目名タップ
```
...
- ワークアウト開始（V5）とワークアウト履歴（V8）はワークアウトタブ内で共存する必要がある。Phase 4ではV5とV8をどう統合/分離するか明示されていなかった
- ~~ワークアウト完了サマリー（V7）からの戻り先が不明確。ワークアウト履歴（V8）またはワークアウト詳細（V9）に遷移するのが自然~~ → V7廃止（#46）。V6完了後にV5+V8へ直接戻り、トースト通知でV9への導線を提供
...
| タブ | 到達先 | 説明 |
|------|--------|------|
| プログラム | プログラム一覧（V1） | プログラムの一覧・作成・複製・削除 |
| ワークアウト | ワークアウト開始 + 履歴（V5/V8） | 新規ワークアウト開始と過去の履歴 |
...
ワークアウトタブは「新規ワークアウト開始」と「過去のワークアウト履歴」の2つの入口を持つ。

```
ワークアウトタブ
  └─ ワークアウト（V5 + V8統合ビュー）
       ├─ ワークアウト開始（V5）: プログラム選択 → Day選択
...
- ワークアウトタブのランディングビューをどうするか: ワークアウト開始（V5）とワークアウト履歴（V8）を1つのビューに統合するか、別ビューにして切り替えるか
- プログラム作成（V14）完了後、プログラム詳細（V2）への自動遷移が必要
- ~~ワークアウト完了サマリー（V7）から次のアクションへの導線: ワークアウト履歴に戻る or ワークアウトタブトップに戻る~~ → V7廃止（#46）。V6完了後にV5+V8へ直接戻り、トースト通知でV9への導線を提供
...
全フェーズの成果物（ユースケース一覧27件、タスク表5Feature/28Function、コンテンツ構造9オブジェクト、コンセプト定義、メンタルモデル5グループ、フレーム構造14単位ビュー、ナビゲーション構造）を横断的に検証した。

### Critical

なし。成果物間の整合性、モードレス原則、プラットフォーム適合性、拡張性のいずれにおいても設計の根幹に関わる問題は検出されなかった。

### Improvement

#### IMP-1: Feature一覧の1RM Function数の不一致

Feature一覧では「1RM: 4」と記載されているが、タスク表の1RMセクションには5つのFunction（1RM登録、1RM更新、e1RM自動算出、e1RM採用、1RM削除）が定義されている。設計判断#8で「1RM登録」と「1RM更新」を分割した結果、Function数が4→5に増えたが、Feature一覧の数値が更新されていない。

→ 修正済み: Feature一覧の1RM Function数を「5」に修正。

#### IMP-2: 「Day構成編集」に旧表現「期間（週数）」が残存

ユースケース一覧（UC_A_3）とタスク表のDay構成編集に「期間（週数）・Day数・各Dayのラベルを定義する」と記載されているが、設計判断#14で「期間（週数）」はプログラムのプロパティから削除されている。操作としてもDay数とラベルの定義が実態。

→ 修正済み: ユースケース一覧とタスク表の両方から「期間（週数）」を削除。

#### IMP-3: 削除系アクションの「…」表記の明示

Phase 4のアクション配置で、削除系アクション（プログラム削除、種目計画削除、セット計画削除、Day削除、ワークアウト削除、種目削除、1RM削除）は確認ダイアログを挟むモーダルなコマンドだが、コマンド名の末尾に「…」が付いていることが明示されていない。モードレス原則チェックリスト項目「モーダルなコマンド名の末尾に『…』が付いているか」に対応する改善。

→ 対応: 本レビューでの記録に留める。削除アクションが確認ダイアログ（モーダル）を伴うことはPhase 4の警告ダイアログ方針で明確だが、実装時にコマンド名に「…」を付与する運用を推奨する。フレーム構造のアクション配置表に注記を追加。

#### IMP-4: 種目選択（V4）の表示形式の補足

V4（種目選択）はDay詳細（V3）から種目追加時に呼び出されるが、フレーム構造の「1. コンテンツ構造からのビュー表現」の表にV4固有の表示形式が記載されていない。種目のビュー表現として「リストビュー」は記載されているが、V4はフィルタ機能付きの選択専用ビューであり、通常の種目一覧（V12）とは異なる。

→ 対応: 本レビューでの記録に留める。V4がフィルタ付き選択ビューであることはPhase 4「2. ユースケース/コンセプトからのビュー表現」の「検索」セクションで言及されている。実装時にV4の表示形式を明確にする。
...
- [x] すべてのタスクがユースケースに対応しているか → 全28FunctionがUCに紐づけ済み
- [x] すべての概念オブジェクトがユースケースまたはタスクに根拠があるか → 9オブジェクト全てに根拠あり
- [x] ナビゲーション経路がコンテンツ構造の多重度と整合しているか → 0..n関連に一覧ビュー、0を含む関連に空表示が対応
- [x] フレーム構造がコンセプト定義に沿っているか → コンテキスト依存の設計方針（計画時: 高密度・俯瞰、記録時: 低密度・集中）がフレーム構造に反映
- [x] メンタルモデルの最重要項目（★）がファーストビューに反映されているか → プログラムタブ（プログラム一覧）がボトムタブの第一項目
- [x] 設計判断ログに根拠のない判断がないか → 全49件にpros/consまたはユーザー確認済みの記載あり

#### 2. モードレス原則

- [x] ファーストビューにオブジェクト（名詞）が並んでいるか → プログラムタブ: プログラム一覧、ワークアウトタブ: ワークアウト開始+履歴
- [x] タブ/メニュー名が概念オブジェクトの名詞か → 「プログラム」「ワークアウト」（設計判断#50, #51で概念オブジェクト名に統一）
- [x] 操作手順が「名詞→動詞」になっているか → プログラム選択→編集、ワークアウト選択→記録等
- [x] インターフェイスをブロックしていないか → フルスクリーンモード廃止（設計判断#33）、タブバー常時表示
- [x] 多重モーダルが2〜3階層以内か → 最大でも確認ダイアログ1階層
- [x] モーダルダイアログにバツボタンがないか → 削除確認は「キャンセル」と「削除」の2択を想定（IMP-3で明示推奨）
- [x] 通知がインライン（バナー等）で控えめか → ワークアウト完了はトースト通知（設計判断#46）
- [x] モーダルなコマンド名の末尾に「…」が付いているか → IMP-3で改善を提案
- [x] ユーザーがモードから脱出できる「避難口」が明確か → キャンセル操作、戻り操作が全ビューで定義済み

#### 3. プラットフォーム適合性

- [x] レイアウトの情報設計原則に沿っているか → iOS: シングルペイン+プッシュ遷移、Web: サイドバー+コンテンツエリア
- [x] プラットフォーム固有のイディオムを活用しているか → iOS: ボトムタブバー、スワイプ削除。Web: サイドバー、マルチペイン
- [x] 新しい操作が1回で覚えられるイディオムになっているか → 標準的なiOS/Webパターンを踏襲
- [x] フォームファクタに適したレイアウト方式を選択しているか → iOS: シングルペイン、Web: マルチペイン（設計判断#28）
- [x] iOSとAndroidのデザインを無理に統一しようとしていないか → Androidは対象外（iOS + Webのみ）
- [x] フローティングツールバーが情報を隠していないか → フローティングツールバーは使用していない
- [x] コンテクストメニュー主体の設計になっていないか → コンテクストメニューは補助的操作（複製・削除）のみ
- [x] ドラッグ＆ドロップや長押しが必須操作になっていないか → 必須操作はタップ/クリックベース

#### 4. 拡張性

- [x] 「1つ」とされているものが本当に1つか疑っているか → 設計判断#13で作成途中の0を許容、#20でアドホックワークアウト（Day紐づけ0..1）を許容
- [x] 空の状態、並び順変更、後からの追加を想定できているか → 概念オブジェクト一覧で空表示要否を定義済み、コンセプト定義で空状態の方針を記載
- [x] 寿命の長い基盤と寿命の短い表層を区別しているか → コンセプト定義（基盤）とカラー・タイポグラフィ（表層）を分離
- [x] パターンが再利用されているか → 計画側/記録側の対称命名（種目計画/種目記録、セット計画/セット記録）、削除時警告パターンの共通化

#### 共通チェックリスト

- [x] すべての設計判断にpros/consが記述されているか → 全52件に記載済み
- [x] 概念オブジェクトの命名にユーザー言語を使っているか → プログラム、Day、種目、ワークアウト等（実装用語なし）
- [x] 多重度が0, 1, nの3値のみで表現されているか → 0..1, 1, 0..n, 1..nのみ使用
- [x] ユースケースが「〜できる」で書かれているか → 全27件が「〜できる」形式
- [x] タブ/メニュー名に動詞やタスク的な名前が使われていないか → 「プログラム」「ワークアウト」（概念オブジェクト名）
- [x] パーキングロットの該当フェーズ項目に対応したか → パーキングロットは全項目解決済み
...
- ~~ワークアウト削除時、e1RM自動算出への影響の扱い~~ → Phase 3で解消（e1RMを導出ビューに変更）
- ~~1RM削除の意味（未設定に戻す）が%1RM指定のセット計画に与える影響~~ → 確定（%1RM指定保持+UI警告+削除時影響通知）
- ~~1RM参照先のセット記録の親ワークアウトが削除された場合の参照の扱い~~ → Phase 4で確定（参照null、値保持 + 削除時警告）
