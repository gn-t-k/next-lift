...
アカウント削除後に同じソーシャルアカウントで再登録した場合、**完全に新規開始**となる。

- 新しいuser IDが発行され、新しいPer-User DBが作成される
- 以前のトレーニングデータへのアクセスは失われる
- 自動データ引き継ぎは行わない

#### 自動引き継ぎをしない理由

メールアドレスやプロバイダーIDによる自動引き継ぎは、以下のリスクがある:

- メールアドレスが別人に再利用された場合、前のユーザーのデータが漏洩する
- プロバイダーのアカウント乗っ取り時に、削除済みデータへのアクセスを与えてしまう

#### 孤立レコードの扱い

削除されたユーザーの`per_user_database`レコードは孤立レコードとして残存する（FK制約がないため）。

- 将来的に、認証DBに紐づかない＋N日経過したPer-User DBを定期削除するバッチで対応予定
- 手動復旧が必要な場合は、運用者がuserIdの紐づけを直接修正するフローで対応する
