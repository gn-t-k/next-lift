...
- **役割**: トレーニング記録の一覧表示
- **関連オブジェクト**: トレーニング記録
- **ナビゲーション**: 記録選択で詳細へ、＋ボタンで作成へ、計画から作成ボタンで計画選択モーダルへ、表示切替アイコンでビュー変更
...
- **備考**: 疲労時でも操作しやすいUI重要、計画選択モーダル（V16）や計画詳細（V8）から遷移した場合は計画データで初期化される
...
- **ナビゲーション**: 編集ボタンで編集へ、実行ボタンで記録作成（V4）へ、記録一覧リンクで記録一覧（V1）へ
- **レイアウト**: ヘッダー（計画名）、予定日のリスト
- **備考**: 進捗状況の可視化、この計画に基づく記録一覧へのリンク
...
- **役割**: 種目の成長推移と履歴表示
- **関連オブジェクト**: 種目、セッション
- **ナビゲーション**: 閉じるボタンで呼び出し元へ戻る、計画一覧リンクで計画一覧（V7）へ
- **レイアウト**: グラフエリア、統計サマリー、セッション履歴リスト
- **備考**: モチベーション管理の要、記録詳細・計画作成/編集から呼び出し可能、この種目を含む計画一覧へのリンク
...
- **役割**: 記録作成時の計画選択
- **関連オブジェクト**: トレーニング計画
- **ナビゲーション**: 選択で記録作成（V4）へ、閉じるで記録一覧（V1）へ
- **レイアウト**: 検索バー、計画リスト
- **備考**: アクティブな計画を上位表示
...
- from: 記録一覧 → trigger: 計画から作成 → to: 計画選択モーダル → 戻り先: 記録一覧
- from: 計画選択モーダル → trigger: 計画選択 → to: 記録作成（初期値付き）→ 戻り先: 記録一覧
...
- from: 記録編集 → trigger: 種目追加 → to: 種目選択モーダル → 戻り先: 記録編集
...
- from: 計画詳細 → trigger: 実行ボタン → to: 記録作成（初期値付き）→ 戻り先: 記録一覧
- from: 計画詳細 → trigger: 記録一覧リンク → to: 記録一覧（フィルター付き）
- from: 計画作成 → trigger: 種目の統計参照 → to: 種目統計（モーダル）→ 戻り先: 計画作成
- from: 計画編集 → trigger: 種目の統計参照 → to: 種目統計（モーダル）→ 戻り先: 計画編集
- from: 種目統計（モーダル）→ trigger: 計画一覧リンク → to: 計画一覧（フィルター付き）
