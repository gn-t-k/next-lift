...
- **CI優先の品質管理**：手動確認を最小化し、なるべくすべてをCIで自動チェック
...
- **形式**: `{type}/{description}`
- **利用可能なtype**: feature/, bugfix/, refactor/, infra/, chore/, deps/, docs/, improve/
- **例**: `feature/user-authentication`, `improve/eslint-config`

### コミットメッセージ

- **必須要素**: どういう理由で何をしたのかを明記
- **形式**: `{理由}のため、{変更内容}を実装`
- **例**: `ユーザー認証機能が必要なため、Better Authの基本設定を追加`

### プルリクエスト作成

- **テンプレート活用**: `.github/pull_request_template.md`の内容にしたがって記述
- **`gh pr create`**: `--body`オプションは使わず、テンプレートをベースに作成
